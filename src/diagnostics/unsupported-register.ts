...
export const UNSUPPORTED_REGISTER: readonly UnsupportedEntry[] = [
  ...
  },
  {
    ...
    what:
      "two Flutter action-body shapes every other frontend renders: a `navigate(…)` / " +
      "`toast(…)` view effect (a Riverpod Notifier holds no BuildContext, so it reaches " +
      "neither the router nor a ScaffoldMessenger) and a `match await` on one of the five " +
