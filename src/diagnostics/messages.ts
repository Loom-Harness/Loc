...
export const DIAGNOSTIC_MESSAGES = {
  ...
  "loom.flutter-action-body-unsupported#view-effect": (p: {
    ...
    `neither the router nor a \`ScaffoldMessenger\`, and the call would be emitted as a comment ` +
    `that silently does nothing. Every other frontend renders it. Move the effect to the ` +
    `widget layer (an \`onClick:\` that navigates directly), or host this ui on another ` +
    `frontend. Tracked as M-T1.32 in docs/new-plan/T1-ui-frontend.md.`,
