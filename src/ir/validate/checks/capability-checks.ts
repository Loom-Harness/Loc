...
function walkReadExprs(s: StmtIR, visit: (e: ExprIR) => void): void {
  switch (s.kind) {
    ...
    case "variant-match":
      ...
      break;
    case "if":
      // Same class as the `variant-match` arm above, caught by this switch's
      // `never` check when M-FT.11 (#2739) added the kind: without this arm a
      // stamp-field read inside an `if` branch never trips the
      // read-before-flush gate.  Condition and both bodies are read positions.
      walkExpr(s.cond, visit);
      for (const st of s.thenBody) walkReadExprs(st, visit);
      for (const st of s.elseBody ?? []) walkReadExprs(st, visit);
      break;
    default: {
