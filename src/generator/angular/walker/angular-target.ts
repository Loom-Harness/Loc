...
export const angularTarget: WalkerTarget = {
  ...
  renderPager(spec) {
    ...
    return (
      ...
      `<button type="button" [disabled]="${p}() <= 1" (click)="${p}.set(${p}() - 1)">${spec.chrome.prevText}</button>` +
      `<span>${spec.chrome.pageOfText(`${p}()`, pages)}</span>` +
      `<button type="button" [disabled]="${p}() >= ${pages}" (click)="${p}.set(${p}() + 1)">${spec.chrome.nextText}</button>` +
      `</div>`
    );
  },
  ...
  buildHookUse(detected: DetectedApiCall, renderArg: (e: ExprIR) => string): TargetHookUse {
    ...
    if (detected.kind === "projection") {
      ...
      // Lives in the shared `../api/projections` module, which Angular emits
      // from its own forked builder (see `angular/projections-module.ts`).
