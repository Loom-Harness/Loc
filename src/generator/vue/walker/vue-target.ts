...
export const vueTarget: WalkerTarget = {
  ...
  renderPager(spec) {
    ...
    return (
      ...
      `<button type="button" :disabled="${p} <= 1" @click="${p} = ${p} - 1">${spec.chrome.prevText}</button>` +
      `<span>${spec.chrome.pageOfText(p, pages)}</span>` +
      `<button type="button" :disabled="${p} >= ${pages}" @click="${p} = ${p} + 1">${spec.chrome.nextText}</button>` +
      `</div>`
    );
  },
  ...
  /** Vue static-attribute escaping — same rule every real static-attr emit
   *  site already shares (`_walker/a11y-emit.ts`'s `escapeHtmlAttr`),
