...
/** `lib/components.dart` plus the per-component BLOCK texts that compose it.
 *
 *  The blocks are what makes a POOLED file mappable: `--sourcemap` records a
 *  whole-file region per emitted file, which would be a lie here (one file, N
 *  components, N different `.ddd` origins), so `index.ts` anchors each block
 *  with `SourceMapRecorder.fragment` instead — a real per-component region,
 *  skipped honestly when the anchor is not unique.  Same `used`/`blocks` order
 *  and the same array length, which is what the anchoring relies on. */
export interface FlutterComponentsFile {
  /** The file content — `""` when no emittable component is used. */
  source: string;
  /** One entry per emitted component, in file order. */
  blocks: { component: ComponentIR; text: string }[];
}
...
export function renderComponentsFile(
  ...
  return {
    source,
    blocks: used.map((c, i) => ({ component: c, text: blocks[i]! })),
  };
}
