...
/** Emit the late Flyway migration when any provenanced field exists: ADD the
 *  co-located `<field>_provenance` jsonb column on each owning aggregate's
 *  (schema-qualified) table.  No-op otherwise (keeps non-provenance projects
 *  byte-identical).  The `provenance_records` history table is NOT here — it
 *  is a shared MigrationsIR companion table (`provenanceTableShape`), created
 *  by the ordinary module migration like the outbox and the audit log. */
export function emitJavaProvenanceMigration(
  ...
  const sql = lines(
    ...
    `-- tables exist for the co-located-column ALTERs.  The provenance_records`,
    `-- history table is NOT here — it is a shared MigrationsIR companion table.`,
    ``,
    ...alters,
    ``,
  );
