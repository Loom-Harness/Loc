...
function renderHandshakeMethods(auth: AuthIR): string[] {
  ...
  return [
    ...
    `        response.addCookie(stateCookie("oidc_verifier", verifier));`,
    ...
    `    }`,
    ``,
    `    /** Exchange the code for tokens + issue the local session cookie. */`,
    `    @GetMapping("${AUTH_BASE_PATH}/callback")`,
    `    public ResponseEntity<?> callback(`,
    `            @RequestParam(required = false) String code,`,
    `            @RequestParam(required = false) String state,`,
    `            @CookieValue(value = "oidc_state", required = false) String expected,`,
    `            @CookieValue(value = "oidc_verifier", required = false) String verifier,`,
    `            HttpServletResponse response) {`,
    `        if (code == null || state == null || !state.equals(expected) || verifier == null) {`,
    ...
    `    @PostMapping("${AUTH_BASE_PATH}/refresh")`,
    ...
    `    }`,
    ``,
    `    /** POST a form-urlencoded token request; null on any non-2xx. */`,
    `    private static JsonNode tokenExchange(String tokenEndpoint, String form) throws Exception {`,
