...
export function renderWireNumberStrictness(basePkg: string): string {
  return lines(
    ...
    ` * too.`,
    ` *`,
    ` * <p>The OTHER direction is the same defect read backwards, and it reaches`,
    ` * every string-typed wire field rather than only the numeric ones:`,
    ` * {@code {"email": 0}} and {@code {"email": false}} deserialized to the`,
    ` * Strings {@code "0"} and {@code "false"} and answered 201, for a body the`,
    ` * published schema declares as {@code type: string}. A {@code money} field`,
    ` * rides the java wire as a String too, so {@code {"amount": 12.5}} was`,
    ` * accepted there by the same coercion. Failing Integer / Float / Boolean →`,
    ` * Textual closes both.`,
    ` */`,
    `@Configuration`,
