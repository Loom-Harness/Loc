...
export function renderWireFormatException(basePkg: string): string {
  return lines(
    ...
    ` * {@code money} and {@code datetime} cross the wire as STRINGS, so a price`,
    ` * of {@code "12,50"} or a {@code placedAt} of {@code ""} is a malformed`,
    ` * REQUEST, not a domain-rule failure.`,
    ` *`,
    ` * <p>Before this existed the service called {@code Instant.parse} /`,
    ` * {@code new BigDecimal(...)} bare and a bad value threw`,
    ` * {@code DateTimeParseException} / {@code NumberFormatException}, which no`,
    ` * advice arm matched: the caller got 500 for input the server itself`,
    ` * refused (schemathesis F19). {@code ApiExceptionAdvice} renders this as`,
    ` * the same 422 + {@code errors[]} envelope Bean Validation and Hono's zod`,
    ` * hook emit, so one malformed field reads identically on every backend.`,
    ` */`,
    `public class WireFormatException extends RuntimeException {`,
    `    /** RFC-6901 pointer to the offending field, e.g. {@code /price}. */`,
    `    private final String fieldPointer;`,
    ``,
    ...
    `    }`,
    ``,
    `    /** Guarded {@code Instant.parse} — the only datetime parse on the wire path. */`,
    `    public static java.time.Instant instant(String pointer, String raw) {`,
    `        try {`,
    `            return java.time.Instant.parse(raw);`,
    `        } catch (RuntimeException e) {`,
    `            throw new WireFormatException(pointer, "Invalid datetime: \\"" + raw + "\\"");`,
    `        }`,
    `    }`,
    ``,
    `    /** Guarded {@code new BigDecimal} — money crosses the wire as a string. */`,
    `    public static java.math.BigDecimal decimal(String pointer, String raw) {`,
    `        try {`,
    `            return new java.math.BigDecimal(raw);`,
    `        } catch (RuntimeException e) {`,
    `            throw new WireFormatException(pointer, "Invalid money: \\"" + raw + "\\"");`,
    `        }`,
    `    }`,
    ``,
    `    /** Guarded {@code UUID.fromString} — an id reference crosses as a string. */`,
    `    public static java.util.UUID uuid(String pointer, String raw) {`,
    `        try {`,
    `            return java.util.UUID.fromString(raw);`,
    `        } catch (RuntimeException e) {`,
    `            throw new WireFormatException(pointer, "Invalid uuid: \\"" + raw + "\\"");`,
    `        }`,
    `    }`,
    `}`,
    ``,
