...
// ---------------------------------------------------------------------------
// Statement LEAF TABLE for the Hono / TypeScript backend.  The 11-kind
// `StmtIR` dispatch, the chunk/join pair and the `variant-match` guard live
// once in `../_stmt/target.ts`; this file supplies only the per-kind spelling
// (audit finding C1).
// ---------------------------------------------------------------------------
...
export function renderTsStatements(
  ...
  return renderStmtsWith(stmts, tsStmtTarget(emitProvenance, traceCtx));
}
...
export function renderTsStatementChunks(
  ...
  return renderStmtChunksWith(stmts, tsStmtTarget(emitProvenance, traceCtx));
}
 ...
 *  frontend-only (never reaches the TS backend — the shared spine's guard
 *  throws) and any future unhandled kind both fall through to `[]`. */
...
/** The Hono leaf table.  Built per call so the arms close over the
 *  provenance switch and the `--trace` context instead of threading them
 *  through the shared spine. */
function tsStmtTarget(emitProvenance: boolean, traceCtx: TraceCtx): StmtTarget {
  return {
    backendName: "TS",
    // PRESERVED INCONSISTENCY (audit C1): node numbers statement temps by
    ...
    precondition: (s, ix) => precondition(s.expr, s.source, s.message, ix.pre, traceCtx),

    requires: (s) =>
      // Authorization gate — surfaces as 403 via the route-level
      // ForbiddenError catch in the per-aggregate routes file.
      `${INDENT}if (!(${renderTsExpr(s.expr)})) throw new ForbiddenError(${JSON.stringify(`Forbidden: ${s.source}`)});`,

    let: (s) =>
      // `let`-names may collide with a JS reserved word; escape consistently
      // with the matching `refKind: "let"` use sites (`let new` → `new_`).
      `${INDENT}const ${escapeTsIdent(s.name)} = ${renderTsExpr(s.expr)};`,

    assign: (s, ix) => {
      const base = `${INDENT}${renderPath(s.target)} = ${renderTsExpr(s.value)};`;
      // NOTE the wrap ORDER: node puts `value_computed` OUTSIDE the provenance
      // capture; .NET / java / python put it inside.  Preserved verbatim — see
      // `withValueComputed`.
      const wrapped = withTrace(base, s.prov, s.target, s.value, emitProvenance, ix.prov);
 ...
 *
 *  PRESERVED INCONSISTENCY (audit C1): node (and python) honour the
 *  `emitProvenance` switch and capture at ANY path depth; .NET and java ignore
 *  the switch (gating on `prov` presence alone) and skip write-throughs into a
 *  containment.  Kept verbatim — reconciling them is a behaviour change on some
...
function withTrace(
  ...
  return wrapProvCapture(base, {
    snapshot: `${INDENT}const ${tmp} = [${inputs}];`,
    lineage: `${INDENT}const ${lin}: ProvLineage = { snapshotId: ${JSON.stringify(prov.snapshotId)}, target: ${targetLit}, inputs: ${tmp}, computedValue: ${computed} };`,
    colocated: `${INDENT}this._${field}_provenance = ${lin};`,
    sink: `${INDENT}this._provTraces.push(${lin});`,
  });
}
