...
export function renderAuditableInterceptor(
  ...
  const usesPrincipal =
    !!actorIdProp &&
    stamping.some(({ rules }) =>
      rules.some((r) => r.assignments.some((a) => exprUsesCurrentUser(a.value))),
    );
...
function renderArm(
  ...
  // access with the `current-user` arm redirected to the SAME ambient accessor
  ...
  const renderValue = (value: ExprIR): string =>
    isCurrentUserRef(value) && actorIdProp
      ...
      : renderCsExpr(value, {
          thisName: argName,
          ...(actorIdProp ? { currentUserExpr: "RequestContext.Current!.CurrentUser!" } : {}),
        });
