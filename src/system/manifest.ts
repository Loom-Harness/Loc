 ...
 *  - **Licence files.**  `ddd generate` used to write a MIT `LICENSE` at the
 *    output root on every run; M-FT.13 stopped it (finding G9 — the licence
 *    belongs to `ddd new`, which owns the project's identity files, not to a
 *    command that emits build output into a tree the user may already have
 *    licensed differently).  Those two changes compose badly without this
 *    clause: every project generated by an older Loom carries `LICENSE` in
 *    its manifest, so the first regen after upgrading would prune it — and
 *    the file it deletes is whatever the user has since put at that path,
 *    which for anyone who swapped in their own terms is a legal document
 *    Loom never authored.  Deleting one is never the right call, so no
 *    licence-shaped file is ever prunable, wherever it sits.
 *
...
export function isProtectedFromPrune(relPath: string): boolean {
  ...
  if (isLicenceFileName(segments[segments.length - 1])) return true;
...
/** True when `fileName` is one of the conventional licence-file names.  The
 *  extension set is the one GitHub and the SPDX tooling recognise; matched
 *  case-insensitively because the convention is upper-case but not enforced.
 *  Loom only ever emitted the bare root `LICENSE`, but the family is
 *  protected wholesale: over-protecting leaves a stale file the user can
 *  delete in a second, while under-protecting silently destroys a legal
 *  document, and the two mistakes are not the same size. */
function isLicenceFileName(fileName: string): boolean {
