...
export class CustomerNewPage {
  ...
  async fill(input: Partial<CreateCustomerRequest>): Promise<this> {
    if (input.name !== undefined) {
      {
        const __f = this.page.getByTestId("customers-new-input-name");
    ...
    if (input.contactEmail !== undefined) {
      {
        const __f = this.page.getByTestId("customers-new-input-contactEmail");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(input.contactEmail!);
      }
    }
    return this;
  }
...
export class CustomerDetailPage {
  ...
  async update(input: UpdateCustomerRequest): Promise<this> {
    ...
    if (input.name !== undefined) {
      {
        const __f = this.page.getByTestId("customers-op-update-input-name");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(input.name!);
      }
    }
    if (input.contactEmail !== undefined) {
      {
        const __f = this.page.getByTestId("customers-op-update-input-contactEmail");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(input.contactEmail!);
      }
    }
