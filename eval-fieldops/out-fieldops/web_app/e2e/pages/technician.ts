...
export class TechnicianNewPage {
  ...
  async fill(input: Partial<CreateTechnicianRequest>): Promise<this> {
    ...
    if (input.fullName !== undefined) {
      {
        const __f = this.page.getByTestId("technicians-new-input-fullName");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(input.fullName!);
      }
    }
    ...
    if (input.costRate !== undefined) {
      {
        const __f = this.page.getByTestId("technicians-new-input-costRate");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(String(input.costRate));
      }
    }
    return this;
  }
...
export class TechnicianDetailPage {
  ...
  async update(input: UpdateTechnicianRequest): Promise<this> {
    ...
    if (input.fullName !== undefined) {
      {
        const __f = this.page.getByTestId("technicians-op-update-input-fullName");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(input.fullName!);
      }
    }
    ...
    if (input.costRate !== undefined) {
      {
        const __f = this.page.getByTestId("technicians-op-update-input-costRate");
        const __i = __f.locator("input, textarea");
        await ((await __i.count()) ? __i.first() : __f).fill(String(input.costRate));
      }
    }
