            ...
            <><div className="loom-table-scroll" style={{ width: "100%", overflowX: "auto" }}><Table striped highlightOnHover stickyHeader>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th><button type="button" style={{ background: "none", border: "none", padding: 0, font: "inherit", cursor: "pointer", userSelect: "none" }} onClick={() => { if (sortKey === "photo") { setSortDir(sortDir === "asc" ? "desc" : "asc"); } else { setSortKey("photo"); setSortDir("asc"); } }}>{t("page.List.columnHeader.n2dhtv", "Photo")}{sortKey === "photo" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}</button></Table.Th>
              ...
              <Table.Tbody>
                { workOrderAll.data.items.map((row) => (
                  <Table.Tr key={ row.id } data-testid={ ("work_orders-row-" + row.id) }>
                    ...
                    <Table.Td>{row.assetId ? (
                    <RouterLink to={`/assets/${ row.assetId }`}><IdValue id={ row.assetId } /></RouterLink>
                  ) : (
                    <span>—</span>
                  )}</Table.Td>
                    <Table.Td>{row.technicianId ? (
                    <RouterLink to={`/technicians/${ row.technicianId }`}><IdValue id={ row.technicianId } /></RouterLink>
                  ) : (
                    <span>—</span>
                  )}</Table.Td>
                    ...
                    <Table.Td>{row.photo ? (
                    <a href={row.photo?.url} download>{row.photo?.key}</a>
                  ) : (
                    <span>—</span>
                  )}</Table.Td>
