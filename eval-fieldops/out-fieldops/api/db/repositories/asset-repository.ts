...
import type { AssetRepositoryPort } from "../../domain/repository-ports.ts";
...
import type { Skill } from "../../domain/value-objects.ts";
...
export class AssetRepository implements AssetRepositoryPort {
  ...
  async findById(id: Ids.AssetId): Promise<Asset | null> {
    return await this.db.transaction(async (tx) => {
      const rootRows = await tx.select().from(schema.assets).where(and(eq(schema.assets.id, id), eq(schema.assets.tenantId, requireCurrentUser().orgId)));
      if (rootRows.length === 0) {
        requestLog().debug({ event: "aggregate_loaded", aggregate: "Asset", id: id as string, found: false });
        return null;
      }
      const root = rootRows[0]!;
      const loaded = Asset._rehydrate({ id: Ids.AssetId(root.id), siteId: Ids.SiteId(root.siteId), serialNumber: root.serialNumber, model: root.model, warrantyExpiry: (root.warrantyExpiry == null ? null : root.warrantyExpiry), requiredSkill: root.requiredSkill as Skill, tenantId: root.tenantId, dataKey: (root.dataKey == null ? null : root.dataKey), version: root.version });
  ...
  async findManyByIds(ids: Ids.AssetId[]): Promise<Asset[]> {
    ...
    const rootRows = await this.db.select().from(schema.assets).where(and(inArray(schema.assets.id, ids), eq(schema.assets.tenantId, requireCurrentUser().orgId)));
    if (rootRows.length === 0) return [];
    return rootRows.map((root) => Asset._rehydrate({ id: Ids.AssetId(root.id), siteId: Ids.SiteId(root.siteId), serialNumber: root.serialNumber, model: root.model, warrantyExpiry: (root.warrantyExpiry == null ? null : root.warrantyExpiry), requiredSkill: root.requiredSkill as Skill, tenantId: root.tenantId, dataKey: (root.dataKey == null ? null : root.dataKey), version: root.version }));
  }
  ...
  async save(aggregate: Asset, expectedVersion?: number): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (existingRow.length === 0) {
        await tx.insert(schema.assets).values(stampInsert({ id: aggregate.id as string, siteId: aggregate.siteId as string, serialNumber: aggregate.serialNumber, model: aggregate.model, warrantyExpiry: aggregate.warrantyExpiry, requiredSkill: aggregate.requiredSkill, tenantId: aggregate.tenantId, dataKey: aggregate.dataKey, version: 1 }));
      } else {
        const updated = await tx.update(schema.assets).set(stampUpdate({ id: aggregate.id as string, siteId: aggregate.siteId as string, serialNumber: aggregate.serialNumber, model: aggregate.model, warrantyExpiry: aggregate.warrantyExpiry, requiredSkill: aggregate.requiredSkill, tenantId: aggregate.tenantId, dataKey: aggregate.dataKey, version: expected + 1 })).where(and(eq(schema.assets.id, aggregate.id), eq(schema.assets.version, expected))).returning({ id: schema.assets.id });
  ...
  async all(page: number, pageSize: number, sort: string, dir: string): Promise<{ items: Asset[]; page: number; pageSize: number; total: number; totalPages: number }> {
    ...
    const sortColumns: Record<string, AnyPgColumn> = { "id": schema.assets.id, "serialNumber": schema.assets.serialNumber, "model": schema.assets.model, "requiredSkill": schema.assets.requiredSkill };
    ...
    const countRows = await this.db.select({ value: count() }).from(schema.assets).where(eq(schema.assets.tenantId, requireCurrentUser().orgId));
    const total = Number(countRows[0]?.value ?? 0);
    const totalPages = pageSize > 0 ? Math.ceil(total / pageSize) : 0;
    const rootRows = await this.db.select().from(schema.assets).where(eq(schema.assets.tenantId, requireCurrentUser().orgId)).orderBy(orderBy).limit(pageSize).offset(offset);
    if (rootRows.length === 0) {
  ...
  toWire(root: Asset): unknown {
    return { id: root.id as string, siteId: root.siteId as string, serialNumber: root.serialNumber, model: root.model, warrantyExpiry: (root.warrantyExpiry == null ? null : (root.warrantyExpiry == null ? null : (root.warrantyExpiry as Date).toISOString().replace(/\.?0+Z$/, "Z"))), requiredSkill: root.requiredSkill as string, version: root.version, display: root.display };
  }

}
//# sourceMappingURL=asset-repository.ts.map
