...
import type { OrganizationRepositoryPort } from "../../domain/repository-ports.ts";
...
export class OrganizationRepository implements OrganizationRepositoryPort {
  ...
  async findById(id: Ids.OrganizationId): Promise<Organization | null> {
    return await this.db.transaction(async (tx) => {
      const rootRows = await tx.select().from(schema.organizations).where(and(eq(schema.organizations.id, id), (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(requireCurrentUser().orgId) ? eq(schema.organizations.id, requireCurrentUser().orgId) : and(isNull(schema.organizations.id), isNotNull(schema.organizations.id)))));
  ...
  async findManyByIds(ids: Ids.OrganizationId[]): Promise<Organization[]> {
    ...
    const rootRows = await this.db.select().from(schema.organizations).where(and(inArray(schema.organizations.id, ids), (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(requireCurrentUser().orgId) ? eq(schema.organizations.id, requireCurrentUser().orgId) : and(isNull(schema.organizations.id), isNotNull(schema.organizations.id)))));
  ...
  async all(page: number, pageSize: number, sort: string, dir: string): Promise<{ items: Organization[]; page: number; pageSize: number; total: number; totalPages: number }> {
    ...
    const countRows = await this.db.select({ value: count() }).from(schema.organizations).where((/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(requireCurrentUser().orgId) ? eq(schema.organizations.id, requireCurrentUser().orgId) : and(isNull(schema.organizations.id), isNotNull(schema.organizations.id))));
    const total = Number(countRows[0]?.value ?? 0);
    const totalPages = pageSize > 0 ? Math.ceil(total / pageSize) : 0;
    const rootRows = await this.db.select().from(schema.organizations).where((/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(requireCurrentUser().orgId) ? eq(schema.organizations.id, requireCurrentUser().orgId) : and(isNull(schema.organizations.id), isNotNull(schema.organizations.id)))).orderBy(orderBy).limit(pageSize).offset(offset);
    if (rootRows.length === 0) {
...
//# sourceMappingURL=organization-repository.ts.map
