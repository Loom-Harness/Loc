...
import type { CustomerRepository } from "../db/repositories/customer-repository.ts";
...
const CreateCustomerRequest = z.object({
  ...
  contactEmail: z.string().refine((s: string) => !s.includes("\u0000")),
...
const AllQuery = z.object({
  ...
  sort: z.enum(["id", "name", "contactEmail", "createdAt", "updatedAt", "createdBy", "updatedBy", ""]).default("id"),
...
export function customerRoutes(repo: CustomerRepository): OpenAPIHono {
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const created = Customer.create({ name: body.name, contactEmail: body.contactEmail });
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      aggregate.update(body.name, body.contactEmail);
...
//# sourceMappingURL=customer.routes.ts.map
