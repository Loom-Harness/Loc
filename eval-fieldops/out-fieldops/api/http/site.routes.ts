...
import type { SiteRepository } from "../db/repositories/site-repository.ts";
...
const AllQuery = z.object({
  ...
  sort: z.enum(["id", "label", "addressLine", ""]).default("id"),
...
export function siteRoutes(repo: SiteRepository): OpenAPIHono {
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const created = Site.create({ customerId: Ids.CustomerId(body.customerId), label: body.label, addressLine: body.addressLine });
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      aggregate.update(Ids.CustomerId(body.customerId), body.label, body.addressLine);
...
//# sourceMappingURL=site.routes.ts.map
