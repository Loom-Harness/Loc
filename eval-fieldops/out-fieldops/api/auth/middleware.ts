...
import type { User, UserClaims } from "./user-types.ts";
import { verifyUserOrThrow } from "./verifier.ts";
...
export const authMiddleware = createMiddleware<{
  ...
  const orgPath = String(claims.orgId ?? "");
  const user: User = { ...claims, permissions: claims.permissions ?? [], orgPath, rootOrg: rootOrgOf(orgPath) };
