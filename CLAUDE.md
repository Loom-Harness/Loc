...
**Which gate catches what — and why `main` can still go red.** Branch protection requires two checks: `tests passed` (the fast vitest rollup, unfiltered on PRs) and **`pr-gate`** (`pr-gate.yml` + `scripts/pr-gate.mjs`) — an event-driven aggregate check: every other workflow's completion (`workflow_run`) triggers a seconds-long evaluation of all checks on the head SHA, posting the `pr-gate` verdict via the Checks API — `failure` if anything triggered failed, `in_progress` while checks still run, `success` when everything triggered passed. Path-skipped workflows never appear on the SHA, so they're OK by construction; anything that *does* trigger — the path-scoped `behavioral-e2e-*` legs, the five `*-obs-e2e` legs, the four native `*-oidc-e2e` legs, the four `generated-*-e2e` SPA smokes, `elixir-vanilla-vo-e2e`, the `pages` build, a `run-*`-label heavy run — is thereby **binding**, without stranding PRs the paths don't match. Re-running a red check re-evaluates automatically; the `workflow_run` list is completeness-pinned by `test/system/pr-gate.test.ts`. The gates that still never run on an unlabeled PR — `tenancy-e2e`, `channels-e2e`, `api-call-e2e`, `migration-evolution-e2e`, `phoenix-ui-e2e`, and the two compose OIDC legs (`elixir-oidc-e2e`, `auth-oidc-compose-e2e`) — remain the post-merge blind spot; wave G1 of [`docs/new-plan/verification-waves-2026-09.md`](docs/new-plan/verification-waves-2026-09.md) promoted the rest and records in its §2/§4 why each of these was deferred (runner-slot cost, in-runner image builds, a flake budget that must close first). **The merge queue is LIVE (2026-09-07)** — the repo moved to the `Loom-Harness` org that day, which lifted GitHub's org-only restriction, and a direct merge is now refused with `405 — Changes must be made through the merge queue`. It requires **22** of the 40 wired gates: the cheap broad set that reads the combined tree, plus the four whose only run it is (`tenancy-e2e`, `migration-evolution-e2e`, and the two compose OIDC legs — the blind spot named just above). The 18 excluded are the docker-booting per-backend legs, which are already binding per-PR and whose queue re-run is redundant given the ruleset's "Require all queue entries to pass required checks". The split is read off each workflow's `if:` guard, not judged, and `merge-queue-readiness.test.ts` fails if a row's claim disagrees with its guard (manifest: `test/system/merge-queue-required-checks.ts`). [`docs/ci-gating.md`](docs/ci-gating.md) documents the tiers, the pr-gate design, and the queue runbook; consult it before assuming a green PR means a green `main`.
