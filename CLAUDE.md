...
**Which gate catches what — and why `main` can still go red.** Branch protection requires two checks: `tests passed` (the fast vitest rollup, unfiltered on PRs) and **`pr-gate`** (`pr-gate.yml` + `scripts/pr-gate.mjs`) — an event-driven aggregate check: every other workflow's completion (`workflow_run`) triggers a seconds-long evaluation of all checks on the head SHA, posting the `pr-gate` verdict via the Checks API — `failure` if anything triggered failed, `in_progress` while checks still run, `success` when everything triggered passed. Path-skipped workflows never appear on the SHA, so they're OK by construction; anything that *does* trigger — the path-scoped `behavioral-e2e-*` legs, the five `*-obs-e2e` legs, the four native `*-oidc-e2e` legs, the four `generated-*-e2e` SPA smokes, `elixir-vanilla-vo-e2e`, the `pages` build, a `run-*`-label heavy run — is thereby **binding**, without stranding PRs the paths don't match. Re-running a red check re-evaluates automatically; the `workflow_run` list is completeness-pinned by `test/system/pr-gate.test.ts`. The gates that still never run on an unlabeled PR — `tenancy-e2e`, `channels-e2e`, `api-call-e2e`, `migration-evolution-e2e`, `phoenix-ui-e2e`, and the two compose OIDC legs (`elixir-oidc-e2e`, `auth-oidc-compose-e2e`) — do not gate your PR; wave G1 of [`docs/new-plan/verification-waves-2026-09.md`](docs/new-plan/verification-waves-2026-09.md) promoted the rest and records in its §2/§4 why each of these was deferred (runner-slot cost, in-runner image builds, a flake budget that must close first). They are no longer a *post-merge* blind spot, though, because the queue below runs them on the rebased candidate. **The merge queue is LIVE** — switched on for `main` on 2026-09-07 (first `merge_group` runs `2026-09-07T11:59:41Z`), and all 39 gates in the required set carry `merge_group:` triggers and stable check names (manifest: `test/system/merge-queue-required-checks.ts`, drift-pinned by `merge-queue-readiness.test.ts`). What this changes for you: **a green PR is no longer the last word** — the queue re-runs the heavy tier on `main` + your PR, so a PR can be green and still be ejected by a gate that never ran on it. Budget for that: an entry runs ~40 checks and needs the queue's wait window (raised 60 → 120 min on 2026-09-08 after every entry for hours was evicted mid-run) to finish. [`docs/ci-gating.md`](docs/ci-gating.md) documents the tiers, the pr-gate design, and the queue runbook; consult it before assuming a green PR means a green `main`.
