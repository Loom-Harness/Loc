...
describe.skipIf(!RUN)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      // jar with one env var changed (CR1-b / P0-4) — env is fixed at boot.
      startBackend = (port, extraEnv) =>
        spawn(javaBin, ["-jar", path.join("build", "libs", jar)], {
          cwd: apiDir,
          env: {
            ...process.env,
    ...
    it("the generated Java OIDC verifier validates a real Keycloak token + maps claims", async () => {
      try {
        ...
        await expectAudienceEnforced({ start: startBackend, token, readyTimeoutMs: 180_000 });
      } catch (err) {
        console.error(`\n===== backend log =====\n${backendLog}\n=======================\n`);
        throw err;
      }
