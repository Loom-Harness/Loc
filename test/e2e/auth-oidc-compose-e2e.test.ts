...
const RUN = declareRunPrecondition({
  suite: "auth OIDC e2e (compose)",
  gate: "LOOM_AUTH_E2E_COMPOSE=1",
  enabled: ENABLED,
  ...
  () => {
    ...
    it("the compose-booted backend validates a real Keycloak token over host.docker.internal", async () => {
      try {
        ...
        expect(
          (await fetch(`${API_BASE}/api/tickets/agent_scoped`, { headers: bearer })).status,
        ).toBe(200);
        ...
        expect(
          (await fetch(`${API_BASE}/api/tickets/admin_scoped`, { headers: bearer })).status,
        ).toBe(403);
