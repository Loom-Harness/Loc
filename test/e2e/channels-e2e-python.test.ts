// M-T4.4 slice 2b — CROSS-BACKEND broker delivery, end to end.
//
// The Python sibling of channels-e2e.test.ts, with the consumer swapped to
// the FastAPI backend: the GENERATED Hono producer (salesApi) publishes a
// CloudEvents envelope to a docker valkey sidecar; the GENERATED Python
// consumer (shipApi) receives it, spawns the correlated `Fulfil` instance,
// and persists the Shipment in its OWN database.  This is the envelope-
// parity proof: two backends, one wire contract, no shared code.
//
// Opt-in: LOOM_CHANNELS_E2E_PYTHON=1 (npm run test:channels-python).
// Needs docker (throwaway postgres + valkey) plus `uv` and npm network
// access for the generated projects' installs.  LOOM_CHANNELS_PG_URL /
// LOOM_CHANNELS_REDIS_URL point at existing services instead.
...
const ENABLED = process.env.LOOM_CHANNELS_E2E_PYTHON === "1";
...
const SALES_PORT = 3185;
const SHIP_PORT = 3186;
const PG_PORT = 55434;
const REDIS_PORT = 56391;
...
describe.skipIf(!ENABLED)("cross-backend broker delivery (channels-e2e, python consumer)", () => {
  let dir: string;
  ...
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-py-"));
    ...
    if (pgOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-py-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-py-pg");
      pgUrl = (db, driver) =>
      ...
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-py-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (redisOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-py-valkey -p ${REDIS_PORT}:6379 valkey/valkey:8-alpine`,
      );
      dockerNames.push("loom-channels-py-valkey");
      redisUrl = `redis://localhost:${REDIS_PORT}`;
    }

    installGeneratedProject(join(dir, "out", "sales_api"), { flags: [], timeout: 420_000 });
    sh("uv sync --quiet", join(dir, "out", "ship_api"));
    ...
    const boot = (
      ...
      // Detached process group so teardown can kill the whole tree (the tsx /
      // uv wrappers spawn the server as a grandchild — see channels-e2e).
    ...
    boot("ship_api", "uv", ["run", "uvicorn", "app.main:app", "--port", String(SHIP_PORT)], {
      DATABASE_URL: pgUrl("ship_api", "asyncpg"),
      LOOM_CHANNEL_LIFECYCLE_BUS_URL: redisUrl,
      PORT: String(SHIP_PORT),
    });
    await waitFor(ready(SALES_PORT), 60_000, "salesApi /ready");
    await waitFor(ready(SHIP_PORT), 120_000, "shipApi (python) /ready");
  }, 600_000);
  ...
  it("delivers a Hono-published envelope into the FastAPI consumer", async () => {
    ...
    await waitFor(
      ...
      20_000,
      "shipment created on the python shipApi",
    );
    await waitFor(
      async () => {
      ...
      10_000,
      "correlated Fulfil instance on the python shipApi",
    );
  }, 120_000);
});
