// M-T4.4 slice 6a — CROSS-BACKEND broker delivery, end to end (.NET leg).
//
// The .NET sibling of channels-e2e-python.test.ts: the GENERATED Hono
// producer (salesApi) publishes a CloudEvents envelope to a docker valkey
// sidecar; the GENERATED .NET consumer (shipApi) receives it, spawns the
// correlated `Fulfil` instance, and persists the Shipment in its OWN
// database — envelope parity three backends deep, no shared code.
//
// Opt-in: LOOM_CHANNELS_E2E_DOTNET=1 (npm run test:channels-dotnet).
// Needs docker, npm network access, and the .NET SDK on PATH (`dotnet run`
// boots the generated consumer — the tenancy-isolation-dotnet pattern).
// LOOM_CHANNELS_PG_URL / LOOM_CHANNELS_REDIS_URL point at existing
// services instead.
...
const SALES_PORT = 3197;
const SHIP_PORT = 3198;
const PG_PORT = 55437;
const REDIS_PORT = 56393;
...
describe.skipIf(!ENABLED)("cross-backend broker delivery (channels-e2e, dotnet consumer)", () => {
  let dir: string;
  const apps: ChildProcess[] = [];
  const dockerNames: string[] = [];
  let pgUrl: (db: string) => string;
  let pgConn: (db: string) => string;
  let redisUrl: string;

  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-dn-"));
    ...
    const toConn = (url: string, db: string): string => {
      const u = new URL(url.replace(/^postgres(ql)?:/, "http:"));
      return `Host=${u.hostname};Port=${u.port || "5432"};Database=${db};Username=${decodeURIComponent(u.username)};Password=${decodeURIComponent(u.password)}`;
    };
    if (pgOverride) {
      pgUrl = (db) => `${pgOverride.replace(/\/[^/]*$/, "")}/${db}`;
      pgConn = (db) => toConn(pgOverride, db);
    } else {
      sh(
        `docker run -d --rm --name loom-channels-dn-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-dn-pg");
      pgUrl = (db) => `postgres://postgres:postgres@localhost:${PG_PORT}/${db}`;
      pgConn = (db) =>
        `Host=localhost;Port=${PG_PORT};Database=${db};Username=postgres;Password=postgres`;
      ...
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-dn-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (redisOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-dn-valkey -p ${REDIS_PORT}:6379 valkey/valkey:8-alpine`,
      );
      dockerNames.push("loom-channels-dn-valkey");
    ...
    boot("ship_api", "dotnet", ["run", "--no-restore", "--no-launch-profile"], {
      ConnectionStrings__Default: pgConn("ship_api"),
      LOOM_CHANNEL_LIFECYCLE_BUS_URL: redisUrl,
      ASPNETCORE_URLS: `http://0.0.0.0:${SHIP_PORT}`,
    });
    await waitFor(ready(SALES_PORT), 60_000, "salesApi /ready");
    await waitFor(ready(SHIP_PORT), 180_000, "shipApi (dotnet) /ready");
  }, 600_000);
  ...
  it("delivers a Hono-published envelope into the .NET consumer", async () => {
    ...
    await waitFor(
      ...
      20_000,
      "shipment created on the dotnet shipApi",
    );
    await waitFor(
      async () => {
      ...
      10_000,
      "correlated Fulfil instance on the dotnet shipApi",
    );
  }, 120_000);
});
