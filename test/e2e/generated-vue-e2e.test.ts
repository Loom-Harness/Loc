...
// ---------------------------------------------------------------------------
// Runtime e2e for the Vue frontend (vue-frontend-plan.md — the docker-boot
// e2e fold-in).
//
// The `generated-vue-build` gate compiles the Vue project (vue-tsc +
// vite build) but never RUNS it — so a runtime-only regression (a broken
// root mount, a nav link that resolves wrong, a named layout that
// compiles but renders empty, a router restructured into nested routes
// that don't match) would slip through.  This boots the built bundle with
...
// backend needed (API calls / the realtime EventSource may fail against no
// server; the smoke only asserts navigation + a visible body).
//
// Opt-in — heavy (npm installs + a Playwright browser download).  Run
// with `LOOM_VUE_E2E=1 npx vitest run test/e2e/generated-vue-e2e.test.ts`
// (or via the generated-vue-e2e.yml CI workflow).  The sibling-gate shape
// matches the Svelte runtime-e2e (generated-svelte-e2e.test.ts).
// ---------------------------------------------------------------------------
...
const ENABLED = process.env.LOOM_VUE_E2E === "1";
const PACK = process.env.LOOM_VUE_E2E_PACK ?? "vuetify@v3";
const EXAMPLE = "examples/vue-showcase.ddd";
...
describe.skipIf(!ENABLED)("generated vue project runs (vite preview + Playwright smoke)", () => {
  it(`${EXAMPLE} × ${PACK}`, { timeout: 900_000 }, async () => {
    const work = fs.mkdtempSync(path.join(os.tmpdir(), "loom-vue-e2e-"));
    ...
    // The vue-showcase frontend deployable is `webApp` → `web_app`.
    const project = path.join(work, "out", "web_app");
    expect(fs.existsSync(path.join(project, "vite.config.ts")), "vue project emitted").toBe(true);
