// M-T4.4 slice 4 — Kafka log semantics on the Hono backend, end to end.
//
// Boots the GENERATED Hono producer plus TWO REPLICAS of the generated Hono
...
const SALES_PORT = 3217;
const REPLICA_PORTS = [3218, 3219];
const PG_PORT = 55445;
const KAFKA_PORT = 55677;
const ORDERS = 6;
...
describe.skipIf(!ENABLED)("kafka log semantics (M-T4.4 slice 4)", () => {
  let dir: string;
  ...
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-kf-"));
    ...
    if (pgOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-kf-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-kf-pg");
      ...
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kf-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (kafkaOverride) {
      ...
      sh(
        `docker run -d --name loom-channels-kf-kafka -p ${KAFKA_PORT}:19092 ` +
      ...
      dockerNames.push("loom-channels-kf-kafka");
      kafkaUrl = `localhost:${KAFKA_PORT}`;
      dlqProbe = true;
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kf-kafka /opt/kafka/bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9092`,
          );
          return true;
        },
        120_000,
  ...
  it("parks a poisoned record on <address>.dlq instead of stalling the partition", async () => {
    ...
    sh(
      `docker exec loom-channels-kf-kafka sh -c 'echo "not-json{{" | /opt/kafka/bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}'`,
    );
    await waitFor(
      async () => {
        const out = sh(
          `docker exec loom-channels-kf-kafka sh -c '/opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}.dlq --from-beginning --timeout-ms 5000 2>/dev/null || true'`,
        );
