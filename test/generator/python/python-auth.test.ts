...
describe("python auth gate", () => {
  it("emits the auth trio: user dataclass, verifier registry, middleware", async () => {
    ...
    // Bypass list matches Hono/.NET exactly — the infra endpoints, plus two
    // surfaces that must be reachable without a principal:
    //   * `/metrics` (F-022): the Prometheus job this same generator writes
    //     carries no credentials, so a gated /metrics 401'd every scrape.
    //   * the dev-only state reset (`src/util/test-reset.ts`), so an
    //     auth-bearing system's e2e suite need not mint a principal just to
    //     empty a table.
    expect(mw).toContain(
      'BYPASS_PREFIXES = ("/health", "/ready", "/metrics", "/openapi.json", "/swagger", "/__loom/test-reset")',
    );
