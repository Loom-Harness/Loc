// Emitted-Python scope checking.
//
// This is agent-B's half of the "does emitted code reference a name it never
// brought into scope?" question, kept in its own module.  `emitted-scope.ts`
// answers the same question for the generated TypeScript backend tree, at FILE
// granularity, over unbound CAPITALISED identifiers; this one answers it for
// generated PYTHON, for one known name, at `def` granularity.
//
// The granularity difference is load-bearing rather than stylistic: a
// file-level check would MISS the motivating defect (F-013).  A system that
// also has a `find ... where ... currentUser...` binds `current_user` in that
// find's signature, so the FILE contains a binding and a file-level scan reads
// green while the retrieval two `def`s down is unbound.
//
// Python-only, deliberately: node/.NET/Java/Elixir reject an unbound name at
// compile time, so their build gates ARE this check.  `python -m compileall`
// does not — Python binds names at execution, so an unbound name is a valid
// module that raises `NameError` on the first request.  This gate exists
// exactly where a compiler does not.
//
// It carries the frontend primitives from `emitted-scope.ts` as a copy rather
// than an import: the two modules are independent test helpers and the shared
// part is small, which is cheaper than coupling them.
