...
describe("G9 — no LICENSE is injected into a generated tree", () => {
  ...
  it("regenerating an OLDER project does not prune the licence it already has", () => {
    // The one genuinely dangerous edge of moving LICENSE off `generate`, and
    // it only exists in combination with M-FT.3's regen pruner (#2735).
    //
    // A project generated before this change carries `LICENSE` in its
    // `.loom/manifest.json`.  The pruner deletes exactly what a past manifest
    // claims and the current run no longer emits — so the first regen after
    // upgrading would delete it, and for anyone who replaced the MIT text
    // with their own terms, the file destroyed is one Loom never wrote.
    // Neither PR could see this alone: #2735 landed while `generate` still
    // emitted the licence (never stale), and this PR stops emitting it
    // without knowing a pruner now exists.  `isProtectedFromPrune` closes it.
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "loom-prune-license-"));
    const src = path.join(dir, "main.ddd");
    fs.writeFileSync(src, SYSTEM);
    const out = path.join(dir, "out");
    const gen = () =>
      spawnSync("node", [cli, "generate", "system", src, "-o", out], { encoding: "utf8" });

    const first = gen();
    expect(first.status, `${first.stdout ?? ""}${first.stderr ?? ""}`).toBe(0);

    // Reconstruct the on-disk state of a project generated by an older Loom:
    // the user's own licence at the root, and a manifest that claims it.
    const OURS = "Copyright (c) 2024 Acme Corp — our terms, not Loom's.\n";
    fs.writeFileSync(path.join(out, "LICENSE"), OURS);
    const manifestPath = path.join(out, ".loom/manifest.json");
    const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
    manifest.entries.push({ path: "LICENSE" });
    manifest.entries.sort((a: { path: string }, b: { path: string }) =>
      a.path.localeCompare(b.path),
    );
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
    // Guard the guard: if the manifest shape ever changes so this seeding
    // stops landing a real entry, the test below would pass vacuously.
    expect(
      JSON.parse(fs.readFileSync(manifestPath, "utf8")).entries.map(
    ...
    expect(second.stdout ?? "").not.toContain("removed             LICENSE");
    expect(fs.existsSync(path.join(out, "LICENSE"))).toBe(true);
    // Byte-for-byte the user's file — not silently replaced with Loom's text.
    expect(fs.readFileSync(path.join(out, "LICENSE"), "utf8")).toBe(OURS);
  });
});
