...
Loom has many test tiers because it generates code for **eleven** targets
(five backends, six frontends) and the interesting failures live at
...
| **Generated frontend runtime** | The built React/Vue/Svelte/Angular bundle actually **runs** — `vite build` (`ng build` for Angular) + preview + the emitted Playwright smoke spec (every param-less route loads). Pure client-side, no backend. | `npm run test:react-e2e` / `:vue-e2e` / `:svelte-e2e` / `:angular-e2e` | `generated-{react,vue,svelte,angular}-e2e` | ✅ path-scoped (each fires on its own framework's emitters, the shared frontend/walker/pack seams and its example + stack — see the file's `pull_request.paths`; it carries a `!src/generator/{java,dotnet,python,elixir}/**` negation tail so a backend-only PR doesn't wake it) |
| **Observability e2e** | The generated backend emits the catalog envelope on stdout (per backend). | `npm run test:obs` (+ `:obs-dotnet/-phoenix/-java/-python`) | `*-obs-e2e.yml` | ✅ path-scoped (`src/ir/**` + that backend's emitters + `src/generator/_obs/**` + its own test file; the per-PR block is narrower than the `push: main` one on purpose) |
| **OIDC auth e2e** | The generated backend really performs the OIDC code flow against a dockerized Keycloak — login → callback → token → `User` mapping — plus the negative-authz half (a `requires`-gated find `403`s the authorized-but-not-permitted caller, `401`s the anonymous one). | `npm run test:auth-e2e` (+ `:-dotnet/-java/-python/-phoenix`, `:-compose`) | `{hono,dotnet,java,python}-oidc-e2e` | ✅ path-scoped. The two **compose** legs (`elixir-oidc-e2e`, `auth-oidc-compose-e2e`) build images inside the runner and stay post-merge / `run-oidc` label. |
...
| **Pairwise corpus** | Generated feature×feature / feature×adapter **crossings** (capability × storage shape × authz × persistence adapter) get an answer, not an exception — plus node `tsc` and `psql -f` schema-load over an all-pairs cover. The intersections the one-fixture-per-feature corpus structurally cannot reach (#2412, #2387/#2391, #2492). See [`docs/audits/pairwise-corpus-findings-2026-08.md`](audits/pairwise-corpus-findings-2026-08.md). | `npm run test:pairwise-corpus` (+ `:-tsc` / `:-schema-load`) | `pairwise.yml` | ⚖️ split: the **generation sweep** is per-PR path-scoped (draft-gated, ~19s, no toolchain); the `tsc` compile oracle and the `psql` schema-load oracle run nightly / on `push: main` / `merge_group` / `run-pairwise` label |
| **Pack spacing — DOM** | The cross-pack spacing contract measured in a real browser (`getComputedStyle`) instead of read off the templates: the distances a library actually renders, plus the phone-width check that no pack scrolls the DOCUMENT sideways. The template half runs in the fast suite (`test/generator/_packs/pack-spacing-contract.test.ts`); this half needs a booted generated app per pack, so it takes the URLs of ones you already have running. | `LOOM_PACK_SPACING_URLS='{"react-mantine":4200}' npm run test:pack-spacing-dom` (or `node scripts/measure-pack-spacing.mjs '{…}' --out measures.json`) | — (local / on-demand) | ❌ opt-in |
...
*Per-PR column verified against the workflow `on:` blocks 2026-09-03, after wave G1 of `docs/new-plan/verification-waves-2026-09.md` promoted the obs, native-OIDC, SPA-smoke and VO legs; the gates still on labels or `push: main` are listed with their deferral reasons in that plan's §2.*
