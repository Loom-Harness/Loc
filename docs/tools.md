...
### When a regen overwrites something you edited

`--dry-run` and `.loomignore` both prevent a hand edit from being lost, but
both require knowing **in advance** that a file is at risk.  So a run that
overwrites a file whose on-disk content was not what Loom last generated
there also **names it**, after the summary:

```
Wrote 5 file(s) in ./out, unchanged: 215
Overwrote 2 locally modified file(s) — the content on disk was not what Loom last generated:
  api/domain/customer.ts
  docker-compose.yml
  Recover the edits from version control, or add a path to .loomignore to keep your version on the next run.
```

This is **not** "every file that changed".  `.loom/manifest.json` records a
content hash for each path the last run wrote, so an ordinary regeneration —
where the model changed and the generated output changed with it — stays
silent, and only a file *somebody else* changed is reported.  A run that
overwrites nothing you touched prints nothing extra.

Two cases qualify: a generated file you edited in place, and a hand-written
file sitting at a path the generator has now started emitting.  A
`.loomignore`d path is never written and so never appears; neither does a
scaffold-once file, which is preserved rather than overwritten.  The report
is advisory — it does not change the exit code, and `--dry-run` previews the
same list as `Would overwrite …` without touching anything.

Output generated by a Loom older than this field carries no hashes, so the
first run after upgrading has nothing to compare against and says nothing;
it records the hashes, and the run after that can tell.
