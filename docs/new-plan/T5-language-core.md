...
- **(a) the MECHANISM has landed; the DENOMINATOR has not.** `src/ir/validate/checks/reserved-surfaces.ts` is the single registry (`RESERVED_SURFACES`) behind one meta-diagnostic, with wording in `messages.ts`, a live `code-docs.ts` anchor, a firing fixture, and a no-stale-rows reachability test (`test/ir/reserved-not-emitted.test.ts`). It carries **3 rows** — `timer-source-timezone`, `timer-source-overlap`, `storage-connection`. What "routed through EVERY parse-but-no-emit surface" still needs is a DENOMINATOR: nothing computes the set of IR fields that no `src/generator/**` or `src/system/**` file reads, so `RESERVED_SURFACES` is a list someone maintains — the shape `gated-features-inventory.md` rotted into. The remaining work is that census (a `test/system` scan over `loom-ir.ts`'s field declarations against downstream references, candidates on a pinned baseline), then one row per confirmed find. Size **S–M**.
...
## M-T5.39 — the declared response record does not describe the wire it claims to — `open` · **L** · P1 ⚠ three live emission defects
