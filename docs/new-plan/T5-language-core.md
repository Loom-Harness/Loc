...
```
and `return c.json(repo.toWire(found) as z.infer<typeof OrderResponse>, 200)` — the cast is why `tsc` never sees it. `aggregate Customer extends Party` is worse: `CustomerResponse` is `{id, tier, version}` while `toWire` emits `{id, name, email, tier, version}`, so every inherited field is missing from the OpenAPI schema and from any client generated off it.
