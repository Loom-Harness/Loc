...
|---|---|
| 2026-09-14-p3-e2e-payload-sweep.md | *(added 2026-09-14)* **No open findings — this is the EVIDENCE for a landed gate, not a findings register.** The corpus sweep behind F4 / packet P3 of the testability-audit fleet: a `test e2e` body's request payload and response-field reads are now checked against `createInput` / the operation's declared params / `wireFieldsForAggregate` (`loom.e2e-unknown-body-key`, `loom.e2e-missing-required-field`, `loom.e2e-body-type-mismatch`, `loom.e2e-unknown-response-field`), and the `e2e-unknown-method` + `e2e-unrouted-verb` double-report is collapsed onto the routing answer. Kept because the number that makes the gate safe is a DENOMINATOR: across all 425 tracked `.ddd` it judges 777 body keys, 671 literals over nine scalar types, 69 enum members and 392 response-field reads and reports zero — so anyone later widening it (the `find`/`list` query-and-envelope arm is the obvious next one, and is deliberately excluded here) has the population to re-measure against rather than a claim to trust. The one defect the sweep found was in the repo's own tests: three inline fixtures whose NON-VACUITY controls asserted "zero diagnostics" over a create body omitting a required `int` — fixed in the same PR |
| 2026-09-13-commons-dev-experience.md | *(added 2026-09-14)* Outside-evaluator dogfooding pass — **Commons**, a customer community platform (members, posts, an arbitrarily-deep comment tree, groups, follows, moderation, bans, media scanning, notification fan-out, GDPR erasure) built from the README and `docs/` only, run against a real Postgres + the generated Keycloak realm, then regenerated onto **all five backends and all six frontends** and compiled with each one's REAL toolchain (elixir via the `scripts/hex-mirror.py` recipe, java via `gradle:9-jdk25`, feliz via `dotnet fable` + vite, flutter via `flutter analyze`), and evolved against a populated database. **23 findings — S1 x10, S2 x6, S3 x6; SILENT 12, HONEST 4.** The 12:4 silent:honest ratio is the headline, and six of the ten S1s are ONE root cause: *a model identifier or construct that is legal in Loom and fatal in the target, interpolated verbatim with no reserved-word map and no post-emit compile check.* **All fixable rows are CLOSED on `main` (2026-09-14):** #2923 (F-014 a record named after a walker primitive skipped construction validation everywhere; F-015 field `state` -> uncompilable C#; F-022 field `member` -> invalid F#; F-023 a workflow + Feliz UI referenced a `Model` field never declared), #2925 (F-001 a cross-context `valueobject` broke emission on NINE targets, plus a second root cause — `indexMembers` never descended `Subdomain.contexts`, so `crudish`'s `update(paid: Money)` lowered to `update(paid: string)` on all five backends; F-020 `mask unless` emitted Java missing `import java.util.Objects`), #2926 (F-013 a reactor `for-each` threw a raw un-coded error that killed the whole `generate system` run; F-021 the same lowering emitted non-compiling Java), #2927 (F-016 `X id[]` -> uncompilable Angular; F-003 the generator emitted an e2e suite calling routes the SAME run did not generate — 405 against the booted stack, incl. the README's own Quick Example; F-004 generated projects ran vitest with no config and inherited an ancestor's), #2924 (F-017 `ddd verify` exited 0 when NOTHING was verified; F-007/F-010/F-011 docs-vs-compiler). Every fix mutation-proved and each S1 additionally proved against the real compiler; each repro re-run against merged `main` with the EMITTED OUTPUT inspected, with the F-016 check used as a before/after control so the rest is evidence rather than a vacuous pass. **Still open:** F-018 (`ddd trace` cannot map the default backend's own bundle) is the only genuinely open row; F-005 is **#2871**'s; F-002/F-012 are expressiveness ceilings, not defects; F-006 (`denyByDefault` does not gate the auto-`findAll` list route) is a posture decision; F-008 (`.loomignore` pin goes permanently stale with no detector) is design; F-009 (no data-preserving move of an aggregate between contexts) is a feature. Carries a **"Found while fixing, deliberately NOT fixed"** section (5 deferred defects, incl. `with crudish, auditable` emitting `createdBy: User id` against a `User` aggregate the model need not declare) and two **corrections to the evaluator's own claims** — F-001's scope was understated because targets were graded by INSPECTION rather than compiled, and four targets were first written off as "unverified — environment" without searching `docs/tools.md`, which ships a recipe named for the exact failure; using it turned two into passes and two into three new S1s |
| 2026-09-10-claimshub-dev-experience.md | *(added 2026-09-13)* Dogfooding pass — an insurance-claims `.ddd` (`ClaimsHub`) built as an ordinary user over three iterations (MVP → auth + `requires` + `auditable` + file upload → multi-tenant + `versioned` + event-triggered `workflow` + `softDelete`), with every stage's emitted **backend and frontend actually `npm install`ed and `tsc --noEmit`'d** rather than trusting a 0-error `generate system`. Models kept beside it at `docs/audits/models/claimshub-v{1,2,3}.ddd`. **Four defects, D1–D4**, deduped at filing time against the three sibling dev-experience sessions (#2861 tracker, #2862 e-shop, #2864 freight). Three carry a fix PR, all **still open** as of 2026-09-13 — the audit's claims remain true on `main` until they land: D1 (the scaffolded Detail page's `Action` hook is handed a `string \| undefined` → TS2345 on react/vue/svelte; reproduces on the repo's own `journey/04-saas.ddd`) → **#2878**; D2 (`currentUser.<undeclared-claim>` types as `string` with no diagnostic and breaks the generated project's `tsc`; the lowering comment promising a validator described one that was never built) → **#2884**; D4 (`policy` and seven siblings were soft only as `LooseName`, so the most obvious field name in the domain was a hintless parse error; `deny` had no identifier position at all) → **#2883** — which also records the half it deliberately does *not* fix, `operation deny()`, since declaration names are bare `ID` and widening them is a separate decision. **D3 → #2965**, which also CORRECTS the audit's own framing of it (the D3 section was rewritten in the same PR): the bypass is real — `crudish`'s generic `update` mass-assigns a field a `requires`-gated operation also writes, skipping the gate *and* the `precondition`, and `with crudish(requires: <Policy>)` (#2877) does not close it — but the remedy the audit proposed (generalize `writableUpdateFields`'s stamp exclusion to auto-drop any field a gated operation assigns) was reviewed and **rejected**: it would silently override a field's *declared* `editable` and add an invisible seventh state to `language.md`'s authoritative six-state access matrix. The state the audit thought was missing **already exists** — `immutable` is read ✓ / create ✓ / update ✗ and is enforced purely wire-side, verified by generating and compiling all five backends, so a gated operation still assigns it. D3 is therefore a **diagnostics + discoverability gap, not a codegen bug**, and #2965 closes it with the advisory `loom.update-gate-suggestion` plus the docs that now say `immutable` constrains the client update input rather than domain assignment. The earlier disposition — "folded onto #2862 slice 1, no mission" — is superseded. The doc's "what worked" half is load-bearing too — it confirms the zero-arg-`criterion` bug of `journey/FINDINGS.md` Friction #3 no longer reproduces. **Scope not covered:** no compose boot / runtime e2e (static `tsc --noEmit` only), node+react only, no i18n |
| 2026-09-03-language-docs-audit-findings.md | *(added 2026-09-03)* The code-side register of the language-docs re-verification pass on `main` @ `651388d` (twelve auditors, one doc packet each): **47 findings, F1–F47**, ranked P0–P4 — 9 silent miscompiles/crashes, 8 silent drops, 7 undeclared cross-backend divergences, 11 diagnostic-catalog hygiene rows, 12 per-feature doc-drift rows — each with a file:line anchor and a repro. The docs were the instrument: these are shapes that parse clean, validate clean and break at emission, which is why the compile-tier gates never saw them. **Fully missioned — nothing unowned:** F2→**M-T5.26**; F5/F6→**M-T5.27**; F1/F4→**M-T5.28**; F36→**M-T5.29**; F3/F9→**M-T1.28**; F10→**M-T1.29**; F12→**M-T1.30**; F11/F17→**M-T1.31**; F7/F8→**M-T6.53**; F18/F19→**M-T6.54**; F14/F15/F24→**M-T6.55**; F16/F20/F22/F23→**M-T6.56**; F21→**M-T6.57** (scoping only — hand to `parity-auditor`); F13→**M-T6.58** (`language-feature-developer`, the register's one missing *feature*); F25/F26/F30→**M-T9.44** (the leverage packet — the catalog gate does not walk the IR check leaves); F27–F29/F31–F35→**M-T9.45**; F37–F46→**M-T9.46** (docs-only, `status-refresh`); F47→**M-T9.47**. Snapshot-in-time: every mission carries ⚠ verify-first, and a finding already fixed closes its packet with a note and no PR. **Wave 1 drained 2026-09-06** (PRs #2786/#2787/#2788/#2789 → M-T1.28, M-T6.53, M-T5.26, M-T5.27): F2/F3/F5/F6/F7/F8/F9 fixed, **five of the seven differed materially from the register** once re-verified on fresh `main` — the doc now carries the corrections inline and three new findings **F48–F50** (50 total). F50 (Svelte's F3+F9 twins) is the one row no packet owns |
| 2026-09-10-independent-completeness-audit.md | *(added 2026-09-10)* A deliberately **fresh** audit — every existing `docs/audits/` file left unread — measured on `main` @ `42fce9e`. Verdict: a finished compiler wrapped in an unfinished product. Records what is genuinely closed (49 of the 61 register `gap` rows cannot fire on any shipping target because the per-backend support sets are all five; 54 of 56 walker primitives carry both a `tsx` and a `heex` renderer; 41 of 46 TODO-family hits in `src/` are prose; zero degradation markers in 1,153 generated files; 2.5 s to generate 753 files) and 13 findings, **F1–F13**. Eight already have owners and were re-derived as a check on the plan (F6→M-T9.55, **fixed by #2843 mid-audit** — kept because the repaired gate still asserts no denominator; F12→M-T9.51, plus the Flutter form-field tail, the persistence-adapter subset, the projection holes, `envelope`, the workflow-`create` miscompile and the `match` error-binding drop). **Five were unowned and are missioned here:** F1 (an aggregate with a field `amount` beside a value object `Amount` is refused on .NET and accepted on the other four — an emitter bug wearing a naming rule) → **M-T6.69**; F2 (the `*-unsupported` register decides MEMBERSHIP by suffix, which its own header argues cannot work — so F1 and `loom.user-component-deferred-target` have no row and no drain) → **M-T9.59**; F3 (the generative fuzzer exists, is **red** on `main`, runs in no workflow, and misattributes a one-backend refusal to its own model generator — so M-T9.22 is mis-statused `open`) → **M-T9.64**; F5 (no advisory or freshness gate on either surface: 11 `npm audit` findings, and four shipping design packs still emit React 18.3 + zod 3.23) → **M-T9.61**; F7 (~40 census gates assert their numerator and not their denominator — the class M-T9.55 is one instance of) → **M-T9.62**; F9 (frontend corpus skew spans 17×, thinnest on the two self-hosting frontends that have no pack-build matrix) → **M-T9.63**; F4 (nothing here is installable: no publish workflow, no `files`/`exports`, six packages at `0.0.0-experimental`, an unpackaged VS Code extension) → **M-T8.24**; F10 (three writers of `.ddd`, no canonical form, though the printer exists) → **M-T8.25**; F11 (`wire-spec.json` is emitted and no command reads it back — zero plan matches for "breaking change") → **M-T2.16**. F13 (45 of 61 register rows unreviewed) folds into M-T9.27. **Two fold-ins, no mission:** `loom.user-component-deferred-target` is a seventh row on M-T1.20's axis, and M-T9.13's `E2E_LESS_CORPUS_FIXTURES` drain should be ordered by runtime risk (`outbox`, `channels-broker`, `resources`, `api-call`, `tenancy-hierarchy`) rather than alphabetically. **Reconciled in §4.0 with [`completion-waves-2026-09.md`](completion-waves-2026-09.md)**, written the same day from the opposite end (it counts denominators that must read zero; this asks what is missing that nothing counts): its §1 LIVE/LATENT split of the register supersedes this doc's cruder count, and its `loom.user-component-deferred-target` row supersedes F2's instance — what survives from F2 is the ENTRY rule, which its table cannot see. Five findings would leave every §1 denominator reading zero unfixed. Mission ids verified free on `93bc82d` and against every open PR at mint (M-T6.69 and M-T2.16 were renumbered off the ids #2840 and #2875 had already claimed). The doc closes with a five-clause **definition of done** — not a competitor to §1's table, the question it cannot ask |
| 2026-09-09-verification-fleet-plan.md | *(added 2026-09-09)* The ORDER for the rest of the register above, plus the six decisions it is blocked on. Eight agents re-verified **30 rows** on `main` @ `adb6b54c0` by GENERATING and reading output rather than reading source: **6 were already fixed, 5 understated their finding, 2 named the wrong symptom, 2 were decisions not defects** — a 50% correction rate, which is the argument for the register's own verify-first rule. They also found **six defects that emit non-compiling or crashing code from `.ddd` reporting `0 error(s), 0 warning(s)`** — F57 (`envelope` does not compile on java/dotnet), F58 (a state-bearing workflow `create` miscompiles on all five), F59 (an `error`-variant binding dropped on dotnet/java), F60 (elixir's app violates its own published OpenAPI), F61 (the HEEx workflow form kills the LiveView), F62 (`DestroyForm` miscompiles on feliz) — plus F63 (the give-up routing gate scans 40 of 140 walker files and reports green), F64 (130 validator conditions collapse onto the non-catalog `loom.unknown`), F65 (every "dropped `workflow_run` dispatch" claim rests on a measurement artifact) and F66 (the gate the four SPA frontends lack already exists, fenced behind a Feliz platform check). **Fully missioned:** F57→**M-T6.57** (escalated P2→P0); F58→**M-T6.60**; F59→**M-T6.61**; F60/F61→**M-T6.56**; F62→**M-T1.31**; F63→**M-T9.55**; F64→**M-T9.56**; F65→**M-T9.57**; F66→**M-T9.44**'s F56 note. **Two don't-fix verdicts recorded so they are not reopened:** F20 (an Ecto `:map` value-object column is a DECISION — identical wire, per-backend databases, already documented) and F28/F29 (two unreachable gates are pinned `LATENT_GATES` dormant safety nets — reword, do not delete). **The recurring cause:** five of the six severe defects survived because no corpus fixture exercises the shape — `envelope` has zero syntactic uses anywhere in the repo — so no compile gate could ever have seen them |
| 2026-09-10-freight-dev-experience.md | *(added 2026-09-10)* Dev-experience audit: a cargo-shipping / freight-forwarding `.ddd` built as an ordinary user over three iterations (booking/routing/handling → tenancy + OIDC + `permissions … implies` + `mask unless` + the `migration {}` blocks to adopt them on a live database → `money`, a `domainService`, two projections, a command-triggered saga, a `channel` and a second Python service), then **compiled** — `npm install` + `tsc --noEmit` on the generated api, and the generated migrations applied to a real `postgres:17`. The model parses **0 errors, 0 warnings** and the generated TypeScript **does not compile**: 17 `tsc` errors from five emitter defects. **Part 2** retargets the identical model to the other four backends and five frontends: **five of the nine do not build**, and three Part-1 findings were under-scoped by measuring one target. Seven defects, four gaps, deduped against the three sibling dev-experience audits (#2861, #2862, #2865) and #2850. Missioned in the fleet plan below — **M-T2.15** (a VO-collection field added to an existing aggregate emits a phantom NOT NULL column that bricks every later insert, proven against postgres), **M-T6.63** (`managed`/`internal` field defaults discarded; `money` defaults dropped on four of five backends), **M-T6.64** (a `valueobject` holding an `X id` emits a file with no imports), **M-T6.65** (an enum-stated workflow references an unemitted `<Enum>Schema` — backend AND four frontends), **M-T6.67a** (a `command`-typed workflow parameter has no wire type on any backend), **M-T1.33** (a nullable `X id?` reference emits an unguarded link on all six frontends; vue and feliz fail to build), **M-T1.34** (Svelte cannot build an aggregate with two operations referencing the same other aggregate), **M-T5.30** (two validator rulings), **M-T9.59** (the unbound-symbol gate), **M-T9.60** (the `money("…")` parser noise and the two contradictory CLI summary lines). **The recurring cause, again:** across 48 generated backend trees and ~1200 TypeScript files the shipped corpus contains no value object holding a cross-aggregate reference, no `user{}` id claim and no enum-stated workflow — so three emitters ship non-compiling output with every gate green |
| 2026-09-10-freight-fleet-plan.md | *(added 2026-09-10, rev. 2)* The ORDER for the audit above, the file ownership that keeps a parallel fleet off the three sibling audits' emitters, and the decisions that gate part of it. Rev. 2 regroups by **root cause** rather than symptom: four findings are one class — *an emitter names a type it never emits or imports* — so the plan leads with **M-T9.59**, the gate for that invariant, landed with one waiver per known site, each fix deleting its own waiver. Thirteen missions in four waves, five concurrent. Four decisions recorded with a recommendation each: **D-1** `handle` continuations (mint the diagnostic now, defer the emitter), **D-2** entity-part parameters (reject in favour of value objects — the payload half needs no ruling and is M-T6.67a), **D-3** field defaults reaching the DDL (emit inside the add-column diff, then `DROP DEFAULT`), **D-4** re-price M-T3.16 now that four independent audits reached for the same workflow workaround and the workflow escape hatch is itself leaky |
| 2026-09-03-language-docs-audit-findings.waves.md | *(added 2026-09-03)* The drain plan for the register above — **18 packets across 7 waves**, sequenced by whether the fix teaches the compiler something reusable rather than by severity (wave 4, the catalog-gate extension, retires eleven findings as a class; wave 2's four findings are one fail-open dispatch predicate in four costumes). Carries the collision protocol (re-verify on fresh `main`, draft-PR claim, disjoint `fileTrees`, one code-minting packet per wave in the shared `src/diagnostics/messages.ts`) and the proof bar (a gate mutation-proved by file copy, an emitter packet compiling its generated project). Machine-readable twin: `2026-09-03-language-docs-audit-findings.waves.json` (per-packet size/priority/trees/`done`). Each packet now names its mission — W1.1→M-T5.26, W1.4→M-T5.27, W3.1→M-T5.28, W3.2→M-T5.29, W1.2→M-T1.28, W2.1→M-T1.29, W2.2→M-T1.30, W2.3→M-T1.31, W1.3→M-T6.53, W5.1→M-T6.54, W5.2→M-T6.55, W5.3→M-T6.56, W5.4→M-T6.57, W6.1→M-T6.58, W4.1→M-T9.44, W4.2→M-T9.45, W7.1→M-T9.46, W7.2→M-T9.47 — so the track files own status from here; the plan doc is a snapshot and loses to them on any disagreement |
...
| numeric-types-audit-2026-08-23.md | *(added 2026-08-23)* Full DB→app→wire→frontend audit of int/long/decimal/money after the #2545→#2560→#2575→#2631 series (five instances of one root cause: the number wire contract re-decided at every read path). F1–F18 + annex: 8 broken-codegen bugs, 6 wire-contract divergences, 1 migrations blindness, 3 structural gate holes; also records what is verifiably stable (money's F4/half-up contract, the shared DDL, the stamped IR). Fully missioned — nothing left unowned: **M-T1.21–M-T1.25** (flutter money, feliz numerics, duplicate `Decimal` import, money-form seams, money display), **M-T2.14** (`columnTypeEqual` precision/scale), **M-T5.22–M-T5.24** (decimal-arithmetic RS ruling, `long` contract, avg-over-money), **M-T6.44–M-T6.48** (leftType-only operand gates, python decimal coherence, java decimal narrowing, dotnet decimal→double hops, numeric ingress 4xx), **M-T9.36–M-T9.38** (wire-codec seam + boundary gate, comparator precision fidelity, flutter/feliz runtime legs). F18 (witness starvation) is distributed — every mission ships its witness. Sibling machine-readable wave plan: `numeric-types-audit-2026-08-23.plan.json` (waves 1–3, conflict map; #2631 claims the dapper slice of F10) |
| targets-completeness-2026-08-30.md | *(added 2026-08-30)* Two-fleet cross-target audit on `38580cd7` (a ledger re-verification pass + an empirical generation pass, 16 agents): **251 open unclaimed rows** — 108 silent (90 proven from emitted output), 40 honest, 65 mission, 26 breadth, 12 stale-prose — plus **5 shipped fixes proven faulty on a neighbouring axis** and 5 cross-fleet contradictions recorded rather than resolved. 49 rows excluded as claimed (#2667's A-series, the numeric fleet #2670–#2678, #2646's own set). Dispositioned into the sibling machine-readable wave plan (`targets-completeness-2026-08-30.waves.json`, 13 packets / 134 P0–P2 rows / 9 sequencing constraints) with the full merged ledger in `targets-completeness-2026-08-30.ledger.json`. Wave 1 (19 rows: the .NET `ignoring *` deny-sentinel leak, dapper seed-schema mismatch, drizzle write-scope shape parity, the elixir `#{}` escape funnel, and the cli/validator foundations) landed in the audit's own PR #2668; remaining packets drain as follow-up PRs. Honest gaps and missions map to existing T1/T5/T6/T9 rows; nothing new minted here |
| targets-completeness-2026-08-17.md | *(added 2026-08-17)* Full cross-target completeness audit (PR #2592): all 24 cross-backend gate sets verified 5/5 uniform; 15 silent cells, 17 honest gaps, 88-entry ratchet inventory, F1–F43. Dispositioned into the sibling machine-readable wave plan (`targets-completeness-2026-08-17.plan.json`): wave-1 correctness fixes in flight as claimed PRs (#2598, #2600, #2603, #2605, …); honest-gap ports map to the existing T6/T1 missions (M-T6.2 §12–14 residue, M-T6.20, M-T6.21, M-T6.30, M-T6.37–M-T6.41, M-T1.18) — the two overturned rows (M-T6.32, M-T6.34) get closure lines from the wave-1 stale-drain PR; breadth/coverage items → T9 (parity-gate matrix extension, schemathesis backend legs, flutter/HEEx behavioral legs) |
| generator-code-review-2026-08-24.md | *(added 2026-08-24)* Seven-pass follow-up review on `cc07658`: 21 new unclaimed bugs (A1–A21 — headline: document-shape × capability-filter aggregations broken on all five backends with a silent EF COUNT leak; drizzle/mikroorm value-on-left predicate inversion; elixir SSE auth bypass; three regressions from the 08-17 remediation window itself), window observations (B), the 08-17 follow-up register re-verified (§C — all 10 items still open, `money[]` confirmed outside #2644's fence), new debt (D), plan rot + open-PR claim hygiene (E), and the architecture queue (F: operand-order normalization at lowering, mode-aware render seams, shared ref-walkers, a realtime plan contract, an i18n round-trip gate). Findings unclaimed at time of writing — drain candidates ranked in §G |
| generator-code-review-2026-08-17.md | *(added 2026-08-18)* Five-pass generator code review: 17 unclaimed bugs (A1–A17), CI structural fixes (B), debt (C), plan rot (D). All unclaimed findings implemented on the review branch itself (fleet, mutation-proved; A8 → #2596, B3 → #2599, B4 = admin click); the doc's §Follow-up register carries the 10 NEW findings the implementation surfaced — EF aggregation `ignoring`, node `money[]` field round-trip, python audit-history F821, the mikroorm no-transaction save path, off-aggregate emit-site atomicity, HEEx testid escaping + Button knob fidelity + tab-slug spaces, the worktree packaging-split failures, and the three preserved StmtTarget leaf inconsistencies — unclaimed, next drain candidates |
...
| **repo-code-review-2026-07-19.md** | Clean disposition table, 10/12 fixed as individual PRs. **F2 FIXED 2026-07-30** (its scope was under-called: **9 templates / 7 packs**, and on React a duplicate `style` prop is TS17001 — a *build-break*, not the wrong-value the table claimed); **F1** (Angular/Feliz silently drop non-`extern` user components — surface/design) → M-T1.4 family; **A1** (= the 07-18 audit's L1) → the L1–L4 member-lookup consolidation below; **I1–I4** (CI gate weaknesses) → M-T9.3/M-T9.8. **G4 is done** (the deployable `serviceSlug`-collision validator ships, `system-checks.ts:656`) |
...
| playground-file-mgmt-review-2026-07.md | Waves 1–3 landed (#2287 — 21 registered defects: store→UI sync epoch, recovered-AST guards, observable file-add, the no-network per-PR Playwright lane, LSP ghost models, write-back parse gates). Its three documented out-of-scope items each got a dedicated mission in #2304: **multi-tab write coordination** (defect #8 — LightningFS superblock last-flush-wins) → **M-T8.12**; the **v1/v2 system-builder endgame decision** → **M-T8.13**; **crash reporting / diagnostics surfacing** → **M-T8.14** |
...
| playground-ux-review-2026-09.md | *(added 2026-09-02)* Usability audit of the playground as shipped (34 findings: 11 high / 17 medium / 6 low) via a production build driven at 1440/1024/800 px + a 390 px phone plus a code sweep of every pane. Its first fix batch (responsive header, correct bundle-failure reporting, registry-unreachable hint, neutral zero badges, disabled-button reasons, platform-aware empty states, two-step Reset database, non-shrinking Requirements badges) landed in the audit's own PR. Every remaining finding is owned by the eight-mission program **M-T8.16–M-T8.23** ([program doc](missions/M-T8.16-playground-ux-program.md) §1 maps finding → mission) |
| playground-landscape-research-2026-09.md | *(added 2026-09-02)* Outward research: the AI app builders (Replit, Lovable, v0, Bolt, Base44, Firebase/AI Studio, Figma Make, Stitch), the compiler playgrounds and the model-driven / low-code tools, plus the human-AI UX guidance, each claim source-linked. Yields the table stakes per family, a 29-row transfer matrix and the target design; its eight-phase roadmap is minted 1:1 as **M-T8.16–M-T8.23**. Deliberately out of scope and recorded as decisions in M-T8.23: short-link service, GitHub push, multiplayer |
...
| pairwise-corpus-findings-2026-08.md | F1 (`shape: document` × `policy` codegen crash) → #2527; F2/F5 (`mask`/principal filters on non-relational repo builders) → #2528; F3 (`mask` × mikroorm) fixed in-PR; F4 (soft-keyword field swallow) → #2529; the harness itself → M-T9.29. **Re-run 2026-08-24 on `main` @ `3a7199c7`** (the first since slice 1): 0 crashes / 0 compile failures / schema-load green, and **four STALE waivers** — F1, F2, F5 had been fixed weeks earlier and their entries never deleted, because the harness had no workflow and the stale arm of the ratchet never fired. `pairwise.yml` wires all three legs (generation per-PR; compile + schema-load nightly/`run-pairwise`); the four waiver registers are now empty. **The compile oracle then reached all five backends** (it was node-only) and found 8 failures — F6/F7/F8 python, F9 dotnet, node+java clean, elixir unverified. TWO are partial fixes of findings this register called CLOSED: F6 **is** F2 (#2528 landed on TypeScript only) and F9 **is** #2527's follow-up 2 (document shape only). Third instance of the same process defect after #2664's three Hono-only schemathesis closures. |
