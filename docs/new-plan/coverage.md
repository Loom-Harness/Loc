...
| code-review-2026-09-13.md | *(added 2026-09-13)* Independent code review of `main` @ `619708fd`, measured on the tree with every existing `docs/audits/` file left unread. Verdict: the CODE is sound (`tsc -b` clean, 24 `as any` and zero `@ts-ignore` in 331k lines, money/SQL-identifier handling correctly centralised, `src/` delete-rate 55% over 60 PRs) and the VERIFICATION/SUPPRESSION layers are where things slipped, in three shapes: time-boxed deferrals became permanent, gates that do not reach what they name, and a test suite that only ratchets upward. **Four P0 findings.** (1) 23 of 114 `ir-walk-census.test.ts` waivers are expired process fences — 12 `INFLIGHT_*` naming PRs #2729/#2736/#2742 (all merged: `dcb956f8`, `90c7e2f2`, `9133a3d6`) and 11 deferring to the 2.6 hotspot split (landed); the ratchet detects a stale SITE, never an expired REASON. (2) 42 more waivers carry `CLOSED_PREDICATE`, whose own text says "classified by default-arm shape, not individually re-verified per kind" — the M-T6.50 class waived on a heuristic; a further 31 `THROWING_DISPATCHER` rows are silent gaps (codegen throws on valid `.ddd`) filed as waivers rather than parity work. (3) The five per-backend compile gates enumerate `src/generator/_expr/**` and NONE of the other 15 shared `src/generator/_*/` dirs — `_stmt/target.ts`, `_workflow/stmt-target.ts`, `_payload/union-wire.ts`, `_type/target.ts`; `corpus-build` (broad `src/generator/**`) is the only net and it excludes Elixir, so a change to the union-wire single-source-of-truth compiles no Elixir project on any per-PR gate. `workflow-path-coverage.test.ts` exempts shared seams as "genuinely per-target" — inverted for the `_*` dirs by construction. (4) OIDC `aud` validation is silently skipped when `audience:` is undeclared on all five backends, with no `loom.*` diagnostic, and hono alone emits no `OIDC_AUDIENCE` env fallback — four backends can enforce it at deploy time and node cannot. **P1:** two e2e suites (677 L) gated on env vars no workflow or script sets (`LOOM_EMBED_E2E_PHOENIX`, `LOOM_AUTH_GATE_E2E`; one orphaned by a workflow rename); `test/` grew +30,765/-1,421 over 60 PRs (4.6% delete rate vs `src/`'s 55%) and 65.8% of 40,463 assertions are `toContain`/`toMatch` on generated text, with `gate-ledger.test.ts`'s drain authority unused; 2 high + 3 moderate `npm audit` advisories with a fix available; 52 dead exports (incl. `generateJava`, superseded by `generateJavaForContexts`) and 417 exported-but-module-local, same class as #2897 and found by hand; `biome ci` exits 0 on 23 auto-fixable warnings. **P2 (decide, do not rush):** 31.8% comment density carrying PR/packet archaeology in the hot path; `docs/` at 43% of `src/` and +20,693 lines per 60 PRs; ~12 `*_SUPPORTED` parity sets now 5-of-5 and unreachable (two genuine gaps remain: `PROJECTION_DOCUMENT_AGG_SUPPORTED` no java, `AUDITED_RETURNING_UNSUPPORTED` node); `packages/` publish-shaped but never exercised out-of-tree. Ordered backlog with effort estimates closes the doc. **No mission minted** — the findings are offered as a backlog for triage, not pre-assigned to tracks |
