...
**Correction (2026-09-03, from the Wave 1 ledger reconciliation).** The paragraph that stood here listed three open P0s (`F2-ADP-1`, `F2-ADP-5`, `F2-ELX-ESCAPE-FUNNEL`). All three — and `ir-warnings-invisible-in-cli`, `F2-VAL-1`, `F2-W-01`, `F2-FFE-6`, `M-T6.26`, the grapheme/code-point row and more — were fixed by **#2668 itself**, the PR that cut the ledger, in the same diff that produced the `open` bucket they still sat in. Verified against source and passing tests by the reconciliation (`docs/audits/targets-completeness-2026-08-30.ledger.json` `done` bucket, with PR numbers; counts now generated by `scripts/ledger-counts.mjs`). After reconciliation the ledger reads: **open 169 — P0 1, P1 11, P2 20, P3 34, P4 91, P5 12**; done 120; claimed 64. The last listed P0, the **`F2-ADP-3` IR-layer gate**, turned out to be closed as well — Wave 1 packet 1a ran the ledger's repro on `b826f87` and got the `loom.dapper-unsupported` refusal W1b (#2708) had landed despite its own "handed off" note. **After Wave 1 (#2752) the ledger has no open P0 and 9 open P1s.** The per-packet residue table above is therefore an upper bound; §2's row lists carry the same overstatement, and every Wave 1 packet verifies each row before building (rule 1), which is what caught this. Lesson: a ledger's `open` bucket is a claim, not a fact — re-derive against tests before scheduling.
...
|---|---|---|---|
| ledger P0 open | 0 after Wave 1 (the plan first said 4) | 0 | 0 |
| ledger P1 silent open | 9 after Wave 1 (the plan first said ~21) | 0 (or handed off with owner) | 0 |
