...
**166 live missions** across the ten track files, and **115** archived — counted by `node scripts/mission-counts.mjs` over the `## M-Tx.y` headings themselves, not typed by hand. By status: `open` 66 · `in-flight` 1 · `partial` 68 · `blocked` 10 · `recurring` 2 · `frozen` 7 · `done` 10 · **2 outside the legend** (M-T5.37 `in progress`, M-T9.42 `in progress`).
...
- **The numeric series is the current cluster.** The 2026-08-23 numeric-types audit minted 17 missions. Both seam missions that were `blocked` behind the frontend money/decimal work have since closed — **M-T9.36** (wire-codec seam) with [#2770](https://github.com/Loom-Harness/Loc/pull/2770) and **M-T9.38** (the Flutter/Feliz runtime legs, now numeric-rich on both) with [#2898](https://github.com/Loom-Harness/Loc/pull/2898). What remains of the series is the RULING cluster — **M-T5.22** (decimal arithmetic is exact; a coordinated single-PR moment because all 54 wire goldens are captured from node), **M-T5.23**/**M-T5.24** (`long` and `avg`-over-money, `blocked(D-LONG-AVG-DEFAULTS)`) — plus **M-T6.60**, whose out-of-range-money arm is an ordinary defect and must NOT wait on its ruling (C1 1e).
