...
**162 live missions** across the ten track files, and **115** archived — counted by `node scripts/mission-counts.mjs` over the `## M-Tx.y` headings themselves, not typed by hand. By status: `open` 65 · `in-flight` 1 · `partial` 67 · `blocked` 10 · `recurring` 2 · `frozen` 7 · `done` 8 · **2 outside the legend** (M-T4.12 `mostly done`, M-T9.42 `in progress`).
