...
**150 live missions** across the ten track files, and **105** archived — counted by `node scripts/mission-counts.mjs` over the `## M-Tx.y` headings themselves, not typed by hand. By status: `open` 53 · `in-flight` 1 · `partial` 65 · `blocked` 14 · `deferred` 1 · `recurring` 2 · `frozen` 7 · `done` 5 · **2 outside the legend** (M-T4.12 `mostly done`, M-T9.42 `in progress`).
