...
**167 live missions** across the ten track files, and **115** archived — counted by `node scripts/mission-counts.mjs` over the `## M-Tx.y` headings themselves, not typed by hand. By status: `open` 67 · `in-flight` 1 · `partial` 68 · `blocked` 10 · `recurring` 2 · `frozen` 7 · `done` 10 · **2 outside the legend** (M-T5.37 `in progress`, M-T9.42 `in progress`).
