...
**Hand-offs:** `CLAUDE.md` still says "re-running a red check does NOT reliably re-evaluate the gate … per the lever table" — refuted (the dispatch was never delivered); M-T9.57 flipped in place, archive move is 0.4's; the "main is red on biome" item did not reproduce on the coordinator's checkout (4 infos, `npm run lint` exit 0).
