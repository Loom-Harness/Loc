...
| E2 non-uuid `{id}` → `Ecto.Query.CastError` ×7 | the CONTROLLER guards it (`plug :__cast_path_id` → 422); the LiveView detail route does not (`get_wallet(socket.assigns.id)` straight into `Repo.get/2`) | **defect** → **M-T6.64** (open, P2) |
...
| # | finding | where |
|---|---|---|
| h1 | the elixir cell's base-path mismatch (E3/E4, and E5's reachability) | **M-T6.63**, `docs/new-plan/T6-backend-parity.md` |
| h2 | a non-UUID id in a LiveView route raises `Ecto.Query.CastError` (500) where the controller answers 422 (E2) | **M-T6.64**, same file |
