...
| 2b .NET / EF / Dapper | Opus | `src/generator/dotnet/**` | `tph-filter-unsupported`; `dapper-unsupported` ×4 + `find-predicate-unsupported` on dapper (build or decide per sub-code); `DAPPER_UNSUPPORTED` tenancy-hierarchy; M-T5.7 `<Concrete>Id`; G2646 dotnet arm; pairwise F12 dotnet; M-T6.14 `HasColumnName` | **ready to fold** ([hand-off](handoffs/wave-c2-2b-dotnet.md)) |
