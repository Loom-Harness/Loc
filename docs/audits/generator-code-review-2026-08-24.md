...
> **Re-verified 2026-09-22 on the Wave C4 base (`0a00c372` = `main` @ `a45fc948b` + the C4 wave log), Wave C4 packet 4a.** One row moves: **F3 → done** (the ledger had already recorded it under #2770; this table was the stale copy). F2 stays `done` and is re-confirmed below. **F4 and F5 are still open** and each now lives on a mission rather than in this table — M-T4.12 (`partial`) and M-T9.39 (`open`) — as does **B3**. The status column below is now "on `0a00c372`"; each row states what was re-read.

| Item | Status on `0a00c372` (re-verified 2026-09-22) | Evidence |
|---|---|---|
...
| F2 — emission *mode* in the shared seam | **done** (Wave 2 packet 2.4, #2770) — re-confirmed on `0a00c372` (`QUERY_EMISSION_MODES`/`QueryEmissionMode` at `_expr/target.ts:470`, `test/generator/_expr/emission-mode.test.ts` present and green) | `src/generator/_expr/target.ts:QueryEmissionMode`/`QUERY_EMISSION_VOCABULARY`/`refuseOutOfVocabulary` is the declared seam; `java/render-jpql.ts:73` `isEntityManagerMode(ctx)` (via `JpqlCtx.mode`) replaces the `if (ctx.principalAccessors)` branch this row named, and every out-of-vocabulary fallback across the query-language renderers (JPQL, `sql-pg-expr.ts`, dotnet dapper raw SQL, python SQLAlchemy, node Drizzle) routes through `refuseOutOfVocabulary` instead of an ad hoc throw. `test/generator/_expr/emission-mode.test.ts`. Ledger `G2667-F2` done |
| F3 — one ref-walker per IR family | **done** (Wave 2 packet 2.3, #2770) | The residue this row named is gone: `collectStmtExprImports` (`src/generator/python/emit/domain-service.ts:256-258`) is now two lines over `walkStmtExprsDeep`, so the hand-enumerated 10-of-11 `StmtIR` switch that row 16 recorded no longer exists (M-T6.50 site 3, verified 2026-09-11). More importantly the CLASS is ratcheted rather than drained once: `test/system/ir-walk-census.test.ts` is a TypeScript-checker-based census of every hand-rolled `switch (x.kind)` over `ExprIR`/`StmtIR`/`WorkflowStmtIR` outside the four sanctioned dispatchers — 133 sites, 17 exhaustive, 116 waived under nine categorised reasons, waivers ratcheting — and `CLAUDE.md` carries the convention ("No hand-rolled IR walks"). Green on this head with no waiver for either python file. Ledger `G2667-F3` done |
| F4 — realtime plan contract | **OPEN — narrowed; owned by M-T4.12 (`partial`)** (subsumes row 20) | Half landed: `src/ir/util/realtime-rooms.ts:154-232` now STATES the contract (RULE 1 — a stream inherits its deployable's auth mode; RULE 2 — it carries the same credential as an ordinary API call) and exposes the single predicate `realtimeStreamCredential(deployable, target, user)` that six frontend emitters call instead of re-deriving it; the live hole row 20 named (no generated SPA could authenticate its own stream) is closed on all six frontends, Flutter last under D-FLUTTER-BEARER. **Still open, per M-T4.12's own body:** (2) the cross-backend conformance test that every SSE route sits behind the auth plug — RULE 1 is asserted at runtime on node only; (3) the durable-event tee at write time (§A9), which stays with M-T4.3; (4) a browser-level runtime leg. Ledger `G2667-F4` → **M-T4.12** |
| F5 — i18n round-trip gate | **OPEN — unchanged; owned by M-T9.39 (`open`)** | #2667 added the A13-*instance* gate `test/generator/_walker/i18n-dead-key-cross-target.test.ts` (its own describes are `A13a` / `A13b`), not the general "every catalog key has a `t(key` consumption site per target, and vice versa" gate. Re-verified on `0a00c372`: the four i18n tests under `test/generator/_walker/` are still `authored-slot-i18n`, `i18n-dead-key-cross-target`, `i18n-extract` and `i18n-slot-inventory` — the general both-directions gate does not exist. **#2668's ledger buckets this as claimed — it is not** → **M-T9.39**, which carries the work and the two-way mutation-proof requirement; it is also queued as improvement-wave row 3.6 and completion-wave row 3f |
| B3 — fold the twin golden-coverage gates | **OPEN — unchanged** | both still exist independently on `0a00c372`: `test/conformance/wire-golden-coverage.test.ts` and `test/behavioral/golden-coverage.test.ts`. Ledger `G2667-B3` |
