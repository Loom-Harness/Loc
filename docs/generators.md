...
| User components | A walked `component <Name>(p: T) { body: … }` emits a standalone `@Component` class at `src/app/components/<Name>.ts` (`components-emit.ts`) — assembled by the SAME shell that assembles a page (`renderAngularPage` in `componentMode`), so state signals, `derived` `computed`s, action methods, api-read hoists, Reactive Forms and the `t` / `FORMAT_HELPERS` / `String` / `Decimal` member lifts all apply.  Params bind as plain `@Input()` fields (bare template reads, like a bound route param; NOT `required: true` — a call site may omit an arg, and the other frontends render that as `undefined`).  A call site uses the component's OWN kebab selector — `<app-tier-badge [label]='tier()'>` — with the class in the page's standalone `imports: []`, since Loom stamped that selector itself; an `extern` component keeps the selector-free `NgComponentOutlet` form (its `@Component({ selector })` is the author's, so Loom has no tag to spell) and its shim sits at `src/components/` rather than `src/app/components/`.  Only the tag form has a content-projection channel, so children passed at a call site — `Panel("a", Text { "child" })` — reach a walked component's `Slot { }` (emitted as `<ng-content>`) and are still dropped on an `extern` one (`loom.component-children-unsupported`, D-ANGULAR-EXTERN-CHILDREN).  Deferred (call site keeps the walker's give-up comment, never a dangling reference): a `slot` / `action(T)` param, and an api read whose ARG reads an `@Input()` (a read hoists as a class-FIELD initializer, which runs before Angular sets inputs; an arg-less read and a reactive find are supported). |
...
| User components | A walked `component <Name>(p: T) { body: … }` emits an F# function of its props record — `let <Name> (props: {| p: T |}) = …`, USED params re-bound as locals — declared in a nested `Components` module (immediately `open`ed) ahead of the page views (F# is order-sensitive; Feliz has no per-component file), on the channel the hoisted `DataGrid` child already rides (`component-emit.ts`).  The module is why a component may be named after a wire record / `Model` / `Api` at all: `App.fs` is ONE F# module and Fable rejects two members of the same name (`error FABLE: Cannot have two module members with same name`), so scoping the members sidesteps every such collision without enumerating the emitter's own names, and the `open` keeps the call site bare.  The single anonymous-record parameter keeps ONE call form for both flavours: the `extern` seam's `<Name> {| … |}` already matches it.  An aggregate / value-object param types from the app's EMITTED wire records, so a record this app doesn't emit is never named.  A READ-bearing body (`QueryView { of: Api.Order.all, … }`) is emitted too: an Elmish read is a field on the ONE `Model` that the init `Cmd` fills, so `readsForUi` collects component bodies alongside page bodies and the function simply takes it — `let RecentOrders (model: Model) (props: {| title: string |}) = … model.AllOrders …`, applied as `RecentOrders model {| title = "Recent" |}` from the page view that was handed `model`.  Deferred (call site keeps the give-up comment): `state {}` / `derived` / named `action`s / forms / store reads, and a `byId` read (its fetch is fired by `pageCmd` on ROUTE entry, keyed to a `Page` case a component has none of) — a Feliz app is ONE Elmish program, so per-component state is an MVU design question (sub-models), not a rendering gap — plus a backstop that drops any component whose rendered F# reaches for `dispatch`, a bare `model`, or a `model.<Field>` the emitted record does not declare. |
...
│   └── migrations/<ts>_create_<table>.exs        # one per aggregate, stable ordering, FK indexes
                                                  # NOTE: no seeds.exs — `seed` datasets emit NOTHING on
                                                  # this backend today (B19 / M-T6.37); the layout adapter
                                                  # reserves the slot but no emitter writes it.
