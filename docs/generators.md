...
| User components | A walked `component <Name>(p: T) { body: … }` emits a standalone `@Component` class at `src/app/components/<Name>.ts` (`components-emit.ts`) — assembled by the SAME shell that assembles a page (`renderAngularPage` in `componentMode`), so state signals, `derived` `computed`s, action methods, api-read hoists, Reactive Forms and the `t` / `FORMAT_HELPERS` / `String` / `Decimal` member lifts all apply.  Params bind as plain `@Input()` fields (bare template reads, like a bound route param; NOT `required: true` — a call site may omit an arg, and the other frontends render that as `undefined`).  Call sites use the same `NgComponentOutlet` form as an `extern` component (Angular has no PascalCase tag; the outlet is selector-free) — only the import path differs: a walked class is a sibling under `src/app/`, an `extern` shim sits at `src/components/`.  Deferred (call site keeps the walker's give-up comment, never a dangling reference): a `slot` / `action(T)` param (`ngComponentOutletInputs` sets inputs and has no content-projection channel), and an api read whose ARG reads an `@Input()` (a read hoists as a class-FIELD initializer, which runs before Angular sets inputs; an arg-less read and a reactive find are supported). |
