...
import type { AssetRepository } from "../db/repositories/asset-repository";
...
const CreateAssetRequest = z.object({
  ...
  requiredSkill: z.string().refine((s: string) => !s.includes("\u0000")),
  serialNumber: z.string().refine((s: string) => !s.includes("\u0000")),
  model: z.string().refine((s: string) => !s.includes("\u0000")),
  warrantyExpiry: z.string().datetime({ offset: true, local: true }).transform((s: string) => new Date(s)),
...
const AllQuery = z.object({
  ...
  sort: z.enum(["id", "requiredSkill", "serialNumber", "model", "warrantyExpiry", ""]).default("id"),
...
export function assetRoutes(repo: AssetRepository): OpenAPIHono {
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const created = Asset.create({ siteId: Ids.SiteId(body.siteId), requiredSkill: body.requiredSkill, serialNumber: body.serialNumber, model: body.model, warrantyExpiry: body.warrantyExpiry });
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      aggregate.update(Ids.SiteId(body.siteId), body.requiredSkill, body.serialNumber, body.model, body.warrantyExpiry);
