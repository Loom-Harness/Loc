...
import type { TechnicianRepository } from "../db/repositories/technician-repository.ts";
...
const CreateTechnicianRequest = z.object({
  ...
  fullName: z.string().refine((s: string) => !s.includes("\u0000")),
  skills: z.array(SkillSchema),
  costRate: moneySchema,
...
const AllQuery = z.object({
  ...
  sort: z.enum(["id", "userId", "fullName", ""]).default("id"),
...
export function technicianRoutes(repo: TechnicianRepository): OpenAPIHono {
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const created = Technician.create({ userId: body.userId, fullName: body.fullName, skills: body.skills.map((e: any) => e), costRate: body.costRate });
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const __maskUser = (c as unknown as { get(k: "currentUser"): import("../auth/user-types.ts").User | undefined }).get("currentUser") ?? null;
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      aggregate.update(body.userId, body.fullName, body.skills.map((e: any) => e), body.costRate);
...
//# sourceMappingURL=technician.routes.ts.map
