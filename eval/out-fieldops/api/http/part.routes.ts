...
import type { PartRepository } from "../db/repositories/part-repository";
...
const AllQuery = z.object({
  ...
  sort: z.enum(["id", "sku", "name", "stockLevel", ""]).default("id"),
...
export const PartResponse = z.object({
  ...
  sku: z.string(),
  name: z.string(),
  stockLevel: z.number().int().openapi({ format: "int32" }),
...
export function partRoutes(repo: PartRepository): OpenAPIHono {
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const created = Part.create({ sku: body.sku, name: body.name, stockLevel: body.stockLevel });
  ...
  app.openapi(
    createRoute({
      ...
      path: "/{id}/decrement",
      tags: ["parts"],
      operationId: "decrementPart",
      request: {
        params: z.object({ id: z.string().uuid() }),
        body: { content: { "application/json": { schema: DecrementPartRequest } } },
      },
      responses: {
        204: { description: "No content" },
        400: { description: "Bad Request", content: { "application/problem+json": { schema: ProblemDetails } } },
    ...
    async (c) => {
      ...
      requireJsonContentType(c);
      const body = c.req.valid("json");
      c.get("log").info({ event: "operation_invoked", aggregate: "Part", op: "decrement", id });
      recordDomainOperation("Part", "decrement");
      const aggregate = await repo.getById(Ids.PartId(id));
      aggregate.decrement(body.qty);
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      aggregate.update(body.sku, body.name, body.stockLevel);
