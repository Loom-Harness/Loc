...
export class PartRepository implements PartRepositoryPort {
  ...
  async findById(id: Ids.PartId): Promise<Part | null> {
    return await this.db.transaction(async (tx) => {
      const rootRows = await tx.select().from(schema.parts).where(and(eq(schema.parts.id, id), eq(schema.parts.tenantId, requireCurrentUser().tenantId)));
      if (rootRows.length === 0) {
        requestLog().debug({ event: "aggregate_loaded", aggregate: "Part", id: id as string, found: false });
        return null;
      }
      const root = rootRows[0]!;
      const loaded = Part._rehydrate({ id: Ids.PartId(root.id), sku: root.sku, name: root.name, stockLevel: root.stockLevel, tenantId: root.tenantId, dataKey: (root.dataKey == null ? null : root.dataKey), version: root.version });
  ...
  async findManyByIds(ids: Ids.PartId[]): Promise<Part[]> {
    ...
    const rootRows = await this.db.select().from(schema.parts).where(and(inArray(schema.parts.id, ids), eq(schema.parts.tenantId, requireCurrentUser().tenantId)));
    if (rootRows.length === 0) return [];
    return rootRows.map((root) => Part._rehydrate({ id: Ids.PartId(root.id), sku: root.sku, name: root.name, stockLevel: root.stockLevel, tenantId: root.tenantId, dataKey: (root.dataKey == null ? null : root.dataKey), version: root.version }));
  }
  ...
  async save(aggregate: Part, expectedVersion?: number): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (existingRow.length === 0) {
        await tx.insert(schema.parts).values(stampInsert({ id: aggregate.id as string, sku: aggregate.sku, name: aggregate.name, stockLevel: aggregate.stockLevel, tenantId: aggregate.tenantId, dataKey: aggregate.dataKey, version: 1 }));
      } else {
        const updated = await tx.update(schema.parts).set(stampUpdate({ id: aggregate.id as string, sku: aggregate.sku, name: aggregate.name, stockLevel: aggregate.stockLevel, tenantId: aggregate.tenantId, dataKey: aggregate.dataKey, version: expected + 1 })).where(and(eq(schema.parts.id, aggregate.id), eq(schema.parts.version, expected))).returning({ id: schema.parts.id });
  ...
  async all(page: number, pageSize: number, sort: string, dir: string): Promise<{ items: Part[]; page: number; pageSize: number; total: number; totalPages: number }> {
    ...
    const sortColumns: Record<string, AnyPgColumn> = { "id": schema.parts.id, "sku": schema.parts.sku, "name": schema.parts.name, "stockLevel": schema.parts.stockLevel };
    ...
    const countRows = await this.db.select({ value: count() }).from(schema.parts).where(eq(schema.parts.tenantId, requireCurrentUser().tenantId));
    const total = Number(countRows[0]?.value ?? 0);
    const totalPages = pageSize > 0 ? Math.ceil(total / pageSize) : 0;
    const rootRows = await this.db.select().from(schema.parts).where(eq(schema.parts.tenantId, requireCurrentUser().tenantId)).orderBy(orderBy).limit(pageSize).offset(offset);
    if (rootRows.length === 0) {
      requestLog().debug({ event: "find_executed", aggregate: "Part", find: "all", rows: 0 });
      return { items: [], page, pageSize, total, totalPages };
    }
    const items = rootRows.map((root) => Part._rehydrate({ id: Ids.PartId(root.id), sku: root.sku, name: root.name, stockLevel: root.stockLevel, tenantId: root.tenantId, dataKey: (root.dataKey == null ? null : root.dataKey), version: root.version }));
    requestLog().debug({ event: "find_executed", aggregate: "Part", find: "all", rows: items.length });
    return { items, page, pageSize, total, totalPages };
  }

  toWire(root: Part): unknown {
    return { id: root.id as string, sku: root.sku, name: root.name, stockLevel: root.stockLevel, version: root.version, display: root.display };
  }
