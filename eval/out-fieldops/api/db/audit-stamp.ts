...
export function stampInsert<T extends Record<string, unknown>>(row: T): T {
  ...
  return { ...row, tenantId: currentUser.tenantId, dataKey: currentUser.orgPath, createdAt: new Date(), createdBy: ctx.actorId, updatedAt: new Date(), updatedBy: ctx.actorId };
}
