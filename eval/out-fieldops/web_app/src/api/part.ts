...
export const CreatePartRequest = z.object({
  sku: z.string(),
  name: z.string(),
  stockLevel: z.number().int().min(0, { message: "Stock Level must be at least 0" }),
});
export type CreatePartRequest = z.infer<typeof CreatePartRequest>;

export const DecrementPartRequest = z.object({
  qty: z.number().int().min(1, { message: "Qty must be at least 1" }),
});
export type DecrementPartRequest = z.infer<typeof DecrementPartRequest>;
...
export function useDecrementPart(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (input: DecrementPartRequest) => {
      await api.post(`/parts/${seg(id)}/decrement`, input);
    },
    onSuccess: () => {
