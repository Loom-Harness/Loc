  ...
  return (
    <Stack gap="md" data-testid="invoices-list">
      ...
      <Group justify="space-between" align="center" gap="md" role="toolbar" aria-label="Actions">
        ...
        <Button onClick={() => navigate("/invoices/new")} data-testid="invoices-list-create">{t("page.List.button.s8g5qi", "New invoice")}</Button>
      </Group>
      <>
            ...
            <><div className="loom-table-scroll" style={{ width: "100%", overflowX: "auto" }}><Table striped highlightOnHover stickyHeader>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th><button type="button" style={{ background: "none", border: "none", padding: 0, font: "inherit", cursor: "pointer", userSelect: "none" }} onClick={() => { if (sortKey === "issued") { setSortDir(sortDir === "asc" ? "desc" : "asc"); } else { setSortKey("issued"); setSortDir("asc"); } }}>{t("page.List.columnHeader.d94nv0", "Issued")}{sortKey === "issued" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}</button></Table.Th>
              ...
              <Table.Tbody>
                { invoiceAll.data.items.map((row) => (
                  <Table.Tr key={ row.id } data-testid={ ("invoices-row-" + row.id) }>
                    ...
                    <Table.Td><Text>{(row.issued ? "Yes" : "No")}</Text></Table.Td>
