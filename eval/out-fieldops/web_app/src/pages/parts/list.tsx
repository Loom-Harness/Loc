            ...
            <><div className="loom-table-scroll" style={{ width: "100%", overflowX: "auto" }}><Table striped highlightOnHover stickyHeader>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th><button type="button" style={{ background: "none", border: "none", padding: 0, font: "inherit", cursor: "pointer", userSelect: "none" }} onClick={() => { if (sortKey === "stockLevel") { setSortDir(sortDir === "asc" ? "desc" : "asc"); } else { setSortKey("stockLevel"); setSortDir("asc"); } }}>{t("page.List.columnHeader.xiiotr", "Stock Level")}{sortKey === "stockLevel" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}</button></Table.Th>
              ...
              <Table.Tbody>
                { partAll.data.items.map((row) => (
                  <Table.Tr key={ row.id } data-testid={ ("parts-row-" + row.id) }>
                    ...
                    <Table.Td><Text>{row.stockLevel}</Text></Table.Td>
