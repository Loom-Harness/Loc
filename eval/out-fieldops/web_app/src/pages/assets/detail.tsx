...
function UpdateForm({ mut, onClose }: { mut: ReturnType<typeof useUpdateAsset>; onClose: () => void }) {
  ...
  const { register, handleSubmit, setError, control, formState: { errors } } = useForm<UpdateAssetRequest>({
    ...
    defaultValues: { siteId: "", requiredSkill: "", serialNumber: "", model: "", warrantyExpiry: "" },
  });
  return (
    <form
    ...
    >
      <Stack>
        ...
        <TextInput label="Required Skill" {...register("requiredSkill")} data-testid="assets-op-update-input-requiredSkill" error={errors.requiredSkill?.message} />
          ...
          <Stack gap="md">
            <Card withBorder padding="md">
              <Stack gap="md">
                ...
                <KeyValueRow label={t("page.Detail.keyValue.3oiuwt", "Required Skill")} data-testid="assets-detail-requiredSkill"><Text>{assetById.data.requiredSkill}</Text></KeyValueRow>
