            ...
            <><div className="loom-table-scroll" style={{ width: "100%", overflowX: "auto" }}><Table striped highlightOnHover stickyHeader>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th><button type="button" style={{ background: "none", border: "none", padding: 0, font: "inherit", cursor: "pointer", userSelect: "none" }} onClick={() => { if (sortKey === "costRatePerHour") { setSortDir(sortDir === "asc" ? "desc" : "asc"); } else { setSortKey("costRatePerHour"); setSortDir("asc"); } }}>{t("page.List.columnHeader.vawwep", "Cost Rate Per Hour")}{sortKey === "costRatePerHour" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}</button></Table.Th>
              ...
              <Table.Tbody>
                { technicianAll.data.items.map((row) => (
                  <Table.Tr key={ row.id } data-testid={ ("technicians-row-" + row.id) }>
                    ...
                    <Table.Td><Text>{row.costRatePerHour}</Text></Table.Td>
