            ...
            <><div className="loom-table-scroll" style={{ width: "100%", overflowX: "auto" }}><Table striped highlightOnHover stickyHeader>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th><button type="button" style={{ background: "none", border: "none", padding: 0, font: "inherit", cursor: "pointer", userSelect: "none" }} onClick={() => { if (sortKey === "city") { setSortDir(sortDir === "asc" ? "desc" : "asc"); } else { setSortKey("city"); setSortDir("asc"); } }}>{t("page.List.columnHeader.42k4ma", "City")}{sortKey === "city" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}</button></Table.Th>
              ...
              <Table.Tbody>
                { siteAll.data.items.map((row) => (
                  <Table.Tr key={ row.id } data-testid={ ("sites-row-" + row.id) }>
                    ...
                    <Table.Td><Text>{row.city}</Text></Table.Td>
