...
import { useAllOrganizations } from "../../api/organization";
...
export default function OrganizationList() {
  ...
  const organizationAll = useAllOrganizations({ page: pageNum, pageSize: 10, sort: sortKey, dir: sortDir });
  return (
    <Stack gap="md" data-testid="organizations-list">
      <Breadcrumbs>
        <Anchor component={RouterLink} to="/">{t("page.List.anchor.n0mxf2", "Home")}</Anchor>
        <Text>{t("page.List.text.1jburf", "Organizations")}</Text>
      </Breadcrumbs>
      <Group justify="space-between" align="center" gap="md" role="toolbar" aria-label="Actions">
        <Title order={2}>{t("page.List.heading.1jburf", "Organizations")}</Title>
        <Button onClick={() => navigate("/organizations/new")} data-testid="organizations-list-create">{t("page.List.button.5hled0", "New organization")}</Button>
      </Group>
      <>
        { organizationAll.isLoading && (
    ...
    <Skeleton key={i} height={ 28 } radius="sm" />
        ...
        { organizationAll.isError && (
          <Alert color="red" variant="light">{t("page.List.alert.dp9xzd", "Couldn't load organizations")}</Alert>
        ) }
        { organizationAll.data && organizationAll.data.items.length === 0 && (
          <Center mih={200}><Text c="dimmed">{t("page.List.empty.6n6n1u", "No organizations yet.")}</Text></Center>
        ) }
        { organizationAll.data && organizationAll.data.items.length > 0 && (
          <Paper p="md">
            <><div className="loom-table-scroll" style={{ width: "100%", overflowX: "auto" }}><Table striped highlightOnHover stickyHeader>
              ...
              <Table.Tbody>
                { organizationAll.data.items.map((row) => (
                  <Table.Tr key={ row.id } data-testid={ ("organizations-row-" + row.id) }>
                    <Table.Td><RouterLink to={`/organizations/${ row.id }`}><IdValue id={ row.id } /></RouterLink></Table.Td>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "0.5rem", marginTop: "0.75rem" }} data-testid="pager"><button type="button" disabled={pageNum <= 1} onClick={() => setPageNum(pageNum - 1)}>{t("chrome.prev", "Prev")}</button><span>{t("chrome.pageOf", "Page {page} of {pages}", { page: pageNum, pages: Math.max(1, organizationAll.data.totalPages) })}</span><button type="button" disabled={pageNum >= Math.max(1, organizationAll.data.totalPages)} onClick={() => setPageNum(pageNum + 1)}>{t("chrome.next", "Next")}</button></div></>
          </Paper>
        ) }
      </>
    </Stack>
