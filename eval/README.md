...
node bin/cli.js generate system eval/fieldops/main.ddd -o /tmp/fo
cd /tmp/fo/api && npm install && npx tsc --noEmit      # node backend: clean

# the S1s, each self-contained:
node bin/cli.js generate system eval/repro/G2-invariant-over-parts-breaks-create.ddd -o /tmp/g2
cd /tmp/g2/api && npm i && npx tsc --noEmit            # F-006
node bin/cli.js generate system eval/repro/migr/v1.ddd -o /tmp/m   # then v2.ddd → F-018
```
